...
function PortalApp({initialUser, allUsersInit}) {
  ...
  const parseFile = (file) => {
    ...
    reader.onload = e => {
      ...
      // Windows/Unix line endings, tab-delimited files
      const parseCSV = (raw) => {
        ...
        while (i < lines.length) {
          const ch = lines[i];
          if (inQ) {
            if (ch==='"' && lines[i+1]==='"') { field+='"'; i+=2; }        // escaped quote
            else if (ch==='"') { inQ=false; i++; }                         // close quote
            else { field+=ch; i++; }
          } else {
            if (ch==='"') { inQ=true; i++; }                               // open quote
            else if (ch===delim) { cur.push(field.trim()); field=""; i++; }// field sep
            else if (ch==="\n") { cur.push(field.trim()); rows.push(cur); cur=[]; field=""; i++; } // row sep
            else { field+=ch; i++; }
          }
        }
        if (field||cur.length) { cur.push(field.trim()); rows.push(cur); }
        return rows.filter(r=>r.some(v=>v)); // drop fully-empty rows
      };

      const rows2d = parseCSV(text);
      if (rows2d.length < 2) { showToast("File appears empty or has only a header row","err"); return; }
      ...
      const matchers = {
        ...
        name:       /^task.?name$|^name$|^activity$|^title$/i,
      ...
      const map = {};
      headers.forEach(c => {
        Object.entries(matchers).forEach(([k,regex]) => { if(!map[k] && regex.test(c.trim())) map[k]=c; });
      });
  ...
  const buildPrev = (rows, fieldMap) => rows.slice(0,8).map((r,i)=>{
    ...
    const nt = /proj/i.test(rawType)?"Project":/inc/i.test(rawType)?"BAU":/chg|change/i.test(rawType)?"Change":/train/i.test(rawType)?"Training":"BAU";
    return {
      _row:       i+2,
  ...
  const runActImport = () => {
    ...
    setTimeout(()=>{
      ...
      setImportStep("done");
      showToast(`${created} activities imported for ${isAdmin?(importTeam||user.team):user.team}`);
    }, 700);
  };
                  ...
                  <Card key={a.id} style={{borderTop:`3px solid ${tc}`,padding:18}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                      <div style={{flex:1,marginRight:8}}>
                        ...
                        <div style={{fontSize:11,color:"var(--text-4)",fontFamily:"var(--font-mono)",marginTop:2}}>{a.id} · {a.type}</div>
                      </div>
                      <SPill s={a.status}/>
                    </div>
      ...
      <div>
        ...
        <div style={{marginBottom:20}}>
          ...
          <div style={{fontSize:12.5,color:"var(--text-3)",marginTop:4,fontFamily:"var(--font-mono)"}}>
            {isAdmin?"ISMS Administrator — All Teams":user.team+" Team Manager"} · {currentMonthLabel()}
          </div>
        </div>
              ...
              {importStep==="upload" && (
                <div>
                  <div
                    ...
                    onDrop={e=>{e.preventDefault();setDragOver2(false);const f=e.dataTransfer.files[0];if(!f)return;if(/\.xlsx?$/i.test(f.name)){showToast("Please save as CSV first: File → Save As → CSV (Comma delimited)","err");return;}parseFile(f);}}
                    ...
                    <input ref={fileRef2} type="file" accept=".csv,.tsv,.txt,.xlsx,.xls" style={{display:"none"}} onChange={e=>{ const f=e.target.files[0]; if(!f) return; if(/\.xlsx?$/i.test(f.name)){ showToast("Please save as CSV first: File → Save As → CSV (Comma delimited)","err"); return; } parseFile(f); }}/>
                  </div>
