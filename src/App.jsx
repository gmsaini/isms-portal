...
function PortalApp({initialUser, allUsersInit}) {
  ...
  const bLog = {actId:"",mins:30,notes:"",date:today(),ticketRef:""};
  ...
  const saveLog = () => {
    ...
    const ticketRef = lForm.ticketRef || act.ticketNo || act.jira || "";
    if(target){
      setLogs(p=>p.map(l=>l.id===target.id?{...l,actId:lForm.actId,activity:act.name,type:act.type,cat:act.cat,mins:+lForm.mins,notes:lForm.notes,date:lForm.date,ticketRef}:l));
      showToast("Entry updated");
    } else {
      setLogs(p=>[{id:uid("TL"),date:lForm.date,userId:user.id,member:user.name,team:act.team,actId:lForm.actId,activity:act.name,type:act.type,cat:act.cat,mins:+lForm.mins,notes:lForm.notes,ticketRef},...p]);
  ...
  const openEditL = l  => { setTarget(l); setLForm({actId:l.actId,mins:l.mins,notes:l.notes,date:l.date,ticketRef:l.ticketRef||""}); setModal("log"); };
    ...
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
        <div>
          ...
          <div style={{fontSize:12.5,color:"var(--text-3)",marginTop:4,lineHeight:1.6,fontFamily:"var(--font-sans)"}}>
      ...
      {isAdmin && <InfoBanner color="#1e40af" bg="var(--brand-light)" border="#bfdbfe" icon="📋">Admin view — read-only. Activities are created and managed by team managers.</InfoBanner>}{isMgr && <InfoBanner color="#1e40af" bg="var(--brand-light)" border="#bfdbfe" icon="📋">You own the activity list for <strong>{user.team}</strong>. Members only see active activities from their own team.</InfoBanner>}
      <div style={{display:"flex",gap:9,flexWrap:"wrap",background:"var(--surface)",border:"1px solid var(--border)",borderRadius:"var(--radius-md)",padding:"11px 14px",marginBottom:14}}>
        ...
        <input placeholder="🔍 Search…" value={actF.search} onChange={e=>setActF(f=>({...f,search:e.target.value}))} style={{flex:1,minWidth:150,padding:"6px 10px",borderRadius:7,border:"1px solid var(--border-2)",fontSize:12}}/>
      </div>
              ...
              {filtActs.length===0 && (
                <tr>
                  <td colSpan={isMgr?14:13} style={{textAlign:"center",padding:50,color:"var(--text-4)",fontSize:13}}>
                    {isMgr?"No activities yet. Click + New Activity.":"No activities found."}
                  </td>
                </tr>
              )}
            </tbody>
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5,fontFamily:"var(--font-sans)"}}>
              <thead>
                <tr>
                  ...
                  <TH c="Team"/><TH c="Type"/><TH c="Time Spent"/><TH c="Notes"/>
              ...
              <tbody>
                {viewLogs.map(l=>{
                  ...
                  return (
                    <tr key={l.id} className="trow-hover" style={{background:isHighlighted?`${tCol(l.team)}06`:""}}>
                      ...
                      <TD s={{fontSize:11,color:"var(--text-2)"}}>{l.type}</TD>
                      <TD><div style={{fontWeight:700,color:"var(--brand)",fontSize:13,fontFamily:"var(--font-mono)"}}>{fmtM(l.mins)}</div><div style={{fontSize:9,color:"var(--text-4)",fontFamily:"var(--font-mono)"}}>{l.mins} min</div></TD>
                ...
                {viewLogs.length===0&&<tr><td colSpan={isMgrOrAdmin?9:8} style={{textAlign:"center",padding:40,color:"var(--text-4)",fontSize:13}}>
  ...
  return (
    <div style={{display:"flex",height:"100vh",fontFamily:"var(--font-sans)",background:"var(--surface-3)",overflow:"hidden",position:"relative"}}>
      ...
      {modal==="log" && (() => {
        ...
        const logSrc = loggable; // loggable already scoped: admin=all, mgr/member=own team non-done
        const isEdit = !!target;
        return (
          <ModalWrap title={isEdit ? "✏️ Edit Time Entry" : "⏱ Log Time"} onClose={closeM} wide>

            {isEdit && <InfoBanner color="#065f46" bg="#f0fdf4" border="#a7f3d0" icon="✅">Editing entry for <strong>{target.member}</strong></InfoBanner>}

            {/* ── Step 1: Activity selector ── */}
            <div style={{marginBottom:16}}>
              <Lbl t="Activity *">
                <select style={sS} value={lForm.actId} onChange={e=>{
                  const act = acts.find(a=>a.id===e.target.value);
                  setLForm(f=>({...f,
                    actId:e.target.value,
                    ticketRef: act ? (act.ticketNo||act.jira||"") : f.ticketRef||"",
                  }));
                }}>
                  <option value="">— Select an activity to log against —</option>
                  {logSrc.map(a=>(
                    <option key={a.id} value={a.id}>[{a.id}] {a.name} · {a.type}{a.ticketNo?" · "+a.ticketNo:""}</option>
                  ))}
                </select>
              </Lbl>
            </div>

            {/* ── Activity detail card (auto-fills on selection) ── */}
            {selAct ? (
              <div style={{background:"var(--surface-2)",border:"1px solid var(--border)",borderRadius:"var(--radius-md)",padding:"14px 16px",marginBottom:16}}>
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12,marginBottom:10}}>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontWeight:700,fontSize:14,color:"var(--text-1)",marginBottom:3}}>{selAct.name}</div>
                    <div style={{fontSize:12,color:"var(--text-3)",lineHeight:1.5}}>{selAct.desc||"No description."}</div>
                  </div>
                  <TPill t={selAct.team}/>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8}}>
                  {[
                    ["Ticket",    selAct.ticketNo||selAct.jira||"—", "var(--font-mono)"],
                    ["JIRA",      selAct.jira||"—",                   "var(--font-mono)"],
                    ["Type",      selAct.type,                        null],
                    ["Category",  selAct.cat,                         null],
                    ["Nature",    selAct.nature||"Proactive",         null],
                  ].map(([label,val,font])=>(
                    <div key={label} style={{background:"var(--surface)",borderRadius:"var(--radius-sm)",padding:"8px 10px",border:"1px solid var(--border)"}}>
                      <div style={{fontSize:9,fontWeight:700,color:"var(--text-4)",textTransform:"uppercase",letterSpacing:1,fontFamily:"var(--font-mono)",marginBottom:3}}>{label}</div>
                      <div style={{fontSize:11.5,fontWeight:600,color:"var(--text-1)",fontFamily:font||"inherit",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{val}</div>
                    </div>
                  ))}
                </div>
                <div style={{marginTop:10,display:"flex",alignItems:"center",gap:8}}>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"var(--text-4)",marginBottom:3}}>
                      <span>Team Progress</span>
                      <span style={{fontFamily:"var(--font-mono)",fontWeight:600,color:"var(--text-2)"}}>
                        {fmtM(minsForAct(selAct.id))} / {fmtM(selAct.estMins)} est.
                      </span>
                    </div>
              ...
              <div style={{background:"var(--surface-2)",border:"1.5px dashed var(--border-2)",borderRadius:"var(--radius-md)",padding:"20px",marginBottom:16,textAlign:"center",color:"var(--text-4)",fontSize:13}}>
                Select an activity above to see its details and log time against it
              </div>
            )}
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              ...
              {/* Time Spent — full width */}
              <div style={{gridColumn:"1/-1"}}>
                <Lbl t={`Time Spent * — ${fmtM(+lForm.mins||0)}`}>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                    <input type="number" min="1" step="15" style={{...iS,width:100,textAlign:"center",fontFamily:"var(--font-mono)",fontSize:16,fontWeight:700,color:"var(--brand)"}}
                      value={lForm.mins} onChange={e=>setLForm(f=>({...f,mins:+e.target.value}))}/>
                    <span style={{fontSize:12,color:"var(--text-3)"}}>minutes</span>
                  </div>
                  <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                    {[[15,"15m"],[30,"30m"],[45,"45m"],[60,"1h"],[90,"1.5h"],[120,"2h"],[180,"3h"],[240,"4h"],[300,"5h"],[480,"8h"]].map(([v,l])=>(
                      <button key={v} onClick={()=>setLForm(f=>({...f,mins:v}))}
                        style={{padding:"5px 11px",borderRadius:"var(--radius-md)",
              ...
              <Lbl t="Ticket / Reference No.">
                <input style={iS} value={lForm.ticketRef||""} placeholder="e.g. CHG-4521 / INC-0891"
                  onChange={e=>setLForm(f=>({...f,ticketRef:e.target.value}))}/>
                {selAct && (selAct.ticketNo||selAct.jira) && (
                  <div style={{fontSize:11,color:"var(--text-4)",marginTop:4}}>
                    Activity ref:&nbsp;
                    <code style={{fontFamily:"var(--font-mono)",background:"var(--surface-3)",padding:"1px 5px",borderRadius:3,cursor:"pointer",color:"var(--brand)"}}
                      onClick={()=>setLForm(f=>({...f,ticketRef:selAct.ticketNo||selAct.jira||""}))}>
                      {selAct.ticketNo||selAct.jira}
                    </code>
                    &nbsp;<span style={{color:"var(--text-4)"}}>← click to copy</span>
                  </div>
                )}
              </Lbl>

              {/* Work Done / Notes — full width */}
              <div style={{gridColumn:"1/-1"}}>
                <Lbl t="Work Done / Notes">
                  <textarea style={{...iS,minHeight:80,resize:"vertical"}} value={lForm.notes}
                    onChange={e=>setLForm(f=>({...f,notes:e.target.value}))}
                    placeholder="Describe what you worked on — what was done, any blockers, next steps…"/>
                </Lbl>
              </div>
            </div>
