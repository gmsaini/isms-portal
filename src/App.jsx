        ...
        <svg viewBox="0 0 100 100" style={{width:size,height:size,flexShrink:0}}>
          ...
          {arcs.map((a,i)=>(
            <path key={i} d={a.path} fill="none" stroke={a.color} strokeWidth={sw}
              strokeLinecap="butt" style={{filter:`drop-shadow(0 1px 2px ${a.color}44)`}}/>
          ))}
          {label && <text x={cx} y={cy-4} textAnchor="middle" fontSize="12" fontWeight="800" fill="#0f172a">{label}</text>}
          {sublabel && <text x={cx} y={cy+9} textAnchor="middle" fontSize="7" fill="#9ca3af">{sublabel}</text>}
        </svg>
      );
    };
    ...
    return (
      <div>
              ...
              return (
                <div key={c.team} style={{marginBottom:11}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
                    <span style={{display:"flex",alignItems:"center",gap:7,fontSize:12,fontWeight:600,color:"#374151"}}>
                      <span style={{width:10,height:10,borderRadius:"50%",background:col,display:"inline-block",flexShrink:0,boxShadow:`0 0 0 2px ${col}30`}}/>
                      {c.team}
                    </span>
                    <div style={{display:"flex",alignItems:"center",gap:7}}>
                      <span style={{fontSize:10,color:"#9ca3af"}}>{fmtM(c.utilMins)}</span>
                      <span style={{fontSize:12,fontWeight:800,color:uc,background:`${uc}15`,padding:"2px 7px",borderRadius:7,minWidth:44,textAlign:"center"}}>{pct}%</span>
                    </div>
                  </div>
                  <div style={{height:8,background:"#f1f5f9",borderRadius:6,overflow:"hidden",position:"relative"}}>
                    <div style={{height:"100%",width:`${Math.min(100,pct)}%`,background:`linear-gradient(90deg,${col}dd,${col})`,borderRadius:6,transition:"width .6s cubic-bezier(.4,0,.2,1)"}}/>
                  </div>
                </div>
              );
            })}
          ...
          <Card>
            ...
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:12}}>
              <div style={{position:"relative",cursor:"pointer"}} onClick={()=>goPage("activities")}>
                <SvgDonut segments={actByStat.filter(s=>s.val>0)} size={120}
                  label={String(dashActs.length)} sublabel="total"/>
              </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1.2fr",gap:14,marginBottom:14}}>
          ...
          {/* Hours by Team/Member — horizontal BarChart */}
          <Card style={{cursor:"pointer"}} onClick={()=>goPage("timelog")}
            ...
            <BarChart
              ...
              data={isAdmin
                ...
                : logsByMember.map(m=>({...m, val:m.val||0})).filter(d=>d.val>0).sort((a,b)=>b.val-a.val)
              }
            />
          </Card>

          {/* Activity Types — horizontal BarChart */}
          <Card>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
              <div>
                <div style={{fontWeight:700,fontSize:13,color:"#0f172a",marginBottom:2}}>Activity Types</div>
                <div style={{fontSize:11,color:"#9ca3af"}}>Count by work type</div>
              </div>
              <span style={{fontSize:10,color:"#9ca3af"}}>click to filter</span>
            </div>
            <BarChart
          ...
          <Card>
            ...
            <div style={{display:"flex",alignItems:"center",gap:14}}>
              <div style={{position:"relative",flexShrink:0}}>
                <SvgDonut segments={logsByCat.map(d=>({...d,val:d.val}))} size={110}
                  label={fmtM(totalMins)} sublabel="total"/>
              </div>
              <div style={{flex:1}}>
                {logsByCat.map(d=>{
                  return (
              ...
              <Card>
                ...
                {topActs.length===0
                  ...
                  : topActs.map((a,i)=>{
                    ...
                    return (
                      <div key={a.id} onClick={()=>setDashDrill({type:"logs",label:`${a.name} — Entries`,data:dashLogs.filter(l=>l.actId===a.id)})}
                        style={{display:"flex",alignItems:"center",gap:10,marginBottom:7,padding:"9px 11px",
                          borderRadius:10,cursor:"pointer",transition:"all .15s",
                          background:i===0?"linear-gradient(135deg,#fffbeb,#fef3c7)":i===1?"#f8fafc":"#f8fafc",
                          border:`1px solid ${i===0?"#fde68a":i===1?"#e8edf5":"#f1f5f9"}`}}
                        onMouseEnter={e=>{e.currentTarget.style.background="#eff6ff";e.currentTarget.style.borderColor="#bfdbfe";e.currentTarget.style.transform="translateX(2px)";}}
                        onMouseLeave={e=>{e.currentTarget.style.background=i===0?"linear-gradient(135deg,#fffbeb,#fef3c7)":i===1?"#f8fafc":"#f8fafc";e.currentTarget.style.borderColor=i===0?"#fde68a":"#f1f5f9";e.currentTarget.style.transform="";}}>
                        <div style={{width:26,height:26,borderRadius:8,background:medalBg[i],display:"flex",alignItems:"center",justifyContent:"center",fontSize:i<3?14:11,flexShrink:0,fontWeight:800,color:i>=3?"#9ca3af":"#fff",boxShadow:i<3?"0 2px 6px rgba(0,0,0,.15)":"none"}}>
                          {i<3?medals[i]:i+1}
                        </div>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:12,fontWeight:700,color:"#0f172a",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.name}</div>
                          <div style={{height:4,background:"#e8edf5",borderRadius:2,overflow:"hidden",marginTop:4}}>
                            <div style={{height:"100%",width:`${Math.round(a.entries/maxE*100)}%`,background:`linear-gradient(90deg,${col}cc,${col})`,borderRadius:2,transition:"width .5s"}}/>
                          </div>
                        </div>
                        <div style={{textAlign:"right",flexShrink:0}}>
                          <div style={{fontSize:15,fontWeight:800,color:col}}>{a.entries}</div>
                          <div style={{fontSize:9,color:"#9ca3af"}}>logs</div>
                        </div>
                      </div>
                    );
                  })
              ...
              <Card>
                ...
                {heavy.length===0
                  ...
                  : heavy.map((a,i)=>{
                    ...
                    const pct = Math.round(a.logged/Math.max(a.estMins,1)*100);
                    return (
                      <div key={a.id} onClick={()=>setDashDrill({type:"logs",label:`${a.name} — Entries`,data:dashLogs.filter(l=>l.actId===a.id)})}
                        style={{marginBottom:8,cursor:"pointer",padding:"8px 10px",borderRadius:9,transition:"all .12s",
                          background:overEst?"#fef2f2":"#f8fafc",
                          border:`1px solid ${overEst?"#fecaca":"transparent"}`}}
                        onMouseEnter={e=>{e.currentTarget.style.background=overEst?"#fee2e2":"#eff6ff";e.currentTarget.style.borderColor=overEst?"#fca5a5":"#bfdbfe";}}
                        onMouseLeave={e=>{e.currentTarget.style.background=overEst?"#fef2f2":"#f8fafc";e.currentTarget.style.borderColor=overEst?"#fecaca":"transparent";}}>
                        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:6,marginBottom:5}}>
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{display:"flex",alignItems:"center",gap:5}}>
                              <span style={{fontSize:11,fontWeight:700,color:"#0f172a",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:130}}>{a.name}</span>
                              {overEst && <span style={{fontSize:9,background:"#dc2626",color:"#fff",padding:"1px 6px",borderRadius:5,fontWeight:700,flexShrink:0}}>OVER</span>}
                            </div>
                            <div style={{fontSize:9,color:"#9ca3af",marginTop:1}}>{a.ticketNo||a.jira||"—"}{isAdmin&&<> · <span style={{color:col,fontWeight:600}}>{a.team}</span></>}</div>
                          </div>
                          <div style={{textAlign:"right",flexShrink:0}}>
                            <div style={{fontSize:14,fontWeight:800,color:overEst?"#dc2626":col}}>{fmtM(a.logged)}</div>
                            <div style={{fontSize:9,color:"#9ca3af"}}>{pct}% est.</div>
                          </div>
                        </div>
                        <div style={{height:6,background:"#e8edf5",borderRadius:4,overflow:"hidden",position:"relative"}}>
                          <div style={{height:"100%",width:`${Math.round(a.logged/maxH*100)}%`,
                            background:overEst?`linear-gradient(90deg,#dc2626,#f87171)`:`linear-gradient(90deg,${col}bb,${col})`,
                            borderRadius:4,transition:"width .5s cubic-bezier(.4,0,.2,1)"}}/>
                          {a.estMins>0&&a.estMins<=maxH && (
                            <div style={{position:"absolute",top:-1,bottom:-1,left:`${Math.min(98,Math.round(a.estMins/maxH*100))}%`,width:2,background:"#475569",opacity:.5,borderRadius:1}}/>
                          )}
                        </div>
                      </div>
                    );
