...
const WNPill = ({w}) => {
  const m = {Request:["#f5f3ff","#5b21b6"],Change:["#fffbeb","#b45309"],Incident:["#fff1f2","#be123c"]};
  const [bg,fg] = m[w]||["#f1f5f9","#475569"];
  return <span className="badge" style={{background:bg,color:fg}}>{w}</span>;
};
...
function PortalApp({initialUser, allUsersInit}) {
  ...
  const bLog = {actId:"",mins:30,notes:"",date:today(),ticketRef:"",nature:"Proactive",workNature:"Change"};
  ...
  const loggable  = useMemo(()=>{
    ...
    return user?.team ? acts.filter(a=>a.team===user.team&&a.status!=="Done") : [];
  },[acts,user,isAdmin]);
  ...
  const saveLog = () => {
    ...
    if(target){
      setLogs(p=>p.map(l=>l.id===target.id?{...l,actId:lForm.actId,activity:act.name,type:act.type,cat:act.cat,mins:+lForm.mins,notes:lForm.notes,date:lForm.date,ticketRef,nature,workNature}:l));
      showToast("Entry updated");
    } else {
      setLogs(p=>[{id:uid("TL"),date:lForm.date,userId:user.id,member:user.name,team:act.team,actId:lForm.actId,activity:act.name,type:act.type,cat:act.cat,mins:+lForm.mins,notes:lForm.notes,ticketRef,nature,workNature},...p]);
  ...
  const openEditL = l  => { setTarget(l); setLForm({actId:l.actId,mins:l.mins,notes:l.notes,date:l.date,ticketRef:l.ticketRef||"",nature:l.nature||"Proactive",workNature:l.workNature||"Change"}); setModal("log"); };
        ...
        <div style={{display:"grid",gridTemplateColumns:"1.4fr 1fr 1fr",gap:14,marginBottom:14}}>
          ...
          <Card>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
              <div>
                <div style={{fontWeight:700,fontSize:13,color:"var(--text-1)",marginBottom:3,letterSpacing:"-.2px"}}>Activity Status</div>
                <div style={{fontSize:11,color:"var(--text-4)",fontFamily:"var(--font-mono)"}}>Breakdown by status</div>
              </div>
              <span style={{fontSize:10,color:"var(--text-4)"}}>click to drill in</span>
            </div>
                ...
                {(()=>{
                  ...
                  return (
                    <svg viewBox={`0 0 ${VW} ${VH}`} style={{width:"100%",maxWidth:220,height:120}}>
                      {/* Track */}
                      <path d={`M ${cx-R} ${cy} A ${R} ${R} 0 0 1 ${cx+R} ${cy}`} fill="none" stroke="var(--surface-3)" strokeWidth={sw}/>
                      {arcs.map((a,i)=>(
                        <path key={i} d={a.path} fill="none" stroke={a.color} strokeWidth={sw}
                          strokeLinecap="butt" style={{filter:`drop-shadow(0 2px 4px ${a.color}66)`}}/>
                      ))}
                      <text x={cx} y={cy-22} textAnchor="middle" fontSize="54" fontWeight="800" fill="var(--text-1)">{dashActs.length}</text>
                      <text x={cx} y={cy+12} textAnchor="middle" fontSize="22" fill="var(--text-4)">activities</text>
                    </svg>
                  );
                })()}
              </div>
              <div style={{width:"100%",display:"flex",flexDirection:"column",gap:5}}>
                {actByStat.filter(s=>s.val>0).map(s=>(
                  <div key={s.label} onClick={()=>s.val>0&&setDashDrill({type:"acts",label:`${s.label} Activities`,data:dashActs.filter(a=>a.status===s.label)})}
                    style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",borderRadius:"var(--radius-sm)",cursor:s.val>0?"pointer":"default",transition:"all .12s",background:"var(--surface-2)"}}
                    onMouseEnter={e=>{if(s.val>0){e.currentTarget.style.background=`${s.color}12`;e.currentTarget.style.transform="translateX(2px)";}}}
                    onMouseLeave={e=>{e.currentTarget.style.background="var(--surface-2)";e.currentTarget.style.transform="";}}>
                    <div style={{width:10,height:10,borderRadius:"50%",background:s.color,flexShrink:0}}/>
                    <span style={{fontSize:12,color:"var(--text-2)",fontWeight:500,flex:1}}>{s.label}</span>
                    <span style={{fontSize:14,fontWeight:800,color:s.color}}>{s.val}</span>
                    <span style={{fontSize:10,color:"var(--text-4)",minWidth:30,textAlign:"right"}}>{Math.round(s.val/Math.max(dashActs.length,1)*100)}%</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1.2fr",gap:14,marginBottom:14}}>
          ...
          <Card>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <div>
                <div style={{fontWeight:700,fontSize:13,color:"var(--text-1)",fontFamily:"var(--font-sans)",letterSpacing:-.3}}>Recent Entries</div>
                <div style={{fontSize:11,color:"var(--text-3)"}}>Latest time logs</div>
              </div>
            ...
            {dashLogs.slice(0,6).map(l=>(
              <div key={l.id} style={{display:"flex",alignItems:"center",gap:9,padding:"6px 0",borderBottom:"1px solid #f8fafc"}}>
                <div style={{width:24,height:24,borderRadius:"var(--radius-sm)",background:`${tCol(l.team)}20`,color:tCol(l.team),display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:800,flexShrink:0}}>{l.member.split(" ").map(n=>n[0]).join("")}</div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:11,fontWeight:600,color:"var(--text-2)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{l.activity.slice(0,22)}{l.activity.length>22?"…":""}</div>
                  <div style={{fontSize:10,color:"var(--text-4)"}}>{l.member.split(" ")[0]} · {l.date.slice(5)}</div>
                </div>
                <div style={{fontWeight:800,color:"var(--brand)",fontSize:12,fontFamily:"var(--font-mono)",flexShrink:0}}>{fmtM(l.mins)}</div>
              </div>
            ))}
          </Card>
        </div>
  ...
  const PageActivities = () => {
    ...
    const [importPrev, setImportPrev] = React.useState([]);
    const [importResult, setImportResult] = React.useState(null);
    const [dragOver2,  setDragOver2]  = React.useState(false);
    const fileRef2 = React.useRef();

    const ACT_FIELDS = [
      {key:"name",       label:"Activity Name",   required:true,  hint:"e.g. Firewall Policy Review"},
      {key:"type",       label:"Type",            required:false, hint:"Project / Change / BAU / Incident / Training"},
      {key:"cat",        label:"Category",        required:false, hint:"Infrastructure Build / Maintenance / Security…"},
      {key:"priority",   label:"Priority",        required:false, hint:"Critical / High / Medium / Low"},
      {key:"estMins",    label:"Est. Hours",      required:false, hint:"Numeric hours (e.g. 8 = 1 day, 40 = 1 week)"},
      {key:"ticketNo",   label:"Ticket / Ref",    required:false, hint:"CHG-xxxx / REQ-xxxx / INC-xxxx"},
      {key:"jira",       label:"JIRA Ref",        required:false, hint:"JIRA issue key"},
      {key:"nature",     label:"Nature",          required:false, hint:"Proactive / Reactive"},
      {key:"workNature", label:"Work Nature",     required:false, hint:"Change / Request / Incident"},
      {key:"desc",       label:"Description",     required:false, hint:"Scope / detail for team members"},
    ];
    ...
    const parseFile = (file) => {
      ...
      reader.onload = e => {
        ...
        const cols = lines[0].split(delim).map(h=>h.replace(/^"|"$/g,"").trim());
        const rows = lines.slice(1).map(line=>{
          const vals = line.split(delim).map(v=>v.replace(/^"|"$/g,"").trim());
          const obj={}; cols.forEach((h,i)=>{ obj[h]=vals[i]||""; }); return obj;
        }).filter(r=>Object.values(r).some(v=>v));
        const matchers={name:/name|title|activity|summary|task|work/i,type:/^type$|^kind$/i,cat:/cat|category/i,priority:/priority|sev/i,estMins:/est|hours|effort|duration|time/i,ticketNo:/ticket|ref|number|id|key|issue/i,jira:/jira/i,nature:/nature|mode/i,workNature:/work.nature|work.type/i,desc:/desc|detail|scope|notes/i};
        const map={};
        cols.forEach(c=>{ Object.entries(matchers).forEach(([k,re])=>{ if(!map[k]&&re.test(c)) map[k]=c; }); });
        setImportCols(cols); setImportRows(rows); setImportMap(map);
        setImportStep("mapping");
      };
      reader.readAsText(file);
    };

    const buildPrev = (rows, map) => rows.slice(0,8).map((r,i)=>{
      const raw=map.estMins?r[map.estMins]:""; const hrs=parseFloat(raw)||8;
      const estMins=hrs<50?Math.round(hrs*60):Math.round(hrs);
      const type=r[map.type]||"BAU";
      const nt=/proj/i.test(type)?"Project":/inc/i.test(type)?"Incident":/chg|change/i.test(type)?"Change":/train/i.test(type)?"Training":"BAU";
      const prio=r[map.priority]||"Medium";
      const np=/crit/i.test(prio)?"Critical":/high/i.test(prio)?"High":/low/i.test(prio)?"Low":"Medium";
      return {_row:i+2,name:r[map.name]||"",type:nt,cat:r[map.cat]||"Maintenance",priority:np,estMins,ticketNo:r[map.ticketNo]||"",jira:r[map.jira]||"",nature:/react/i.test(r[map.nature]||"")?"Reactive":"Proactive",workNature:/inc/i.test(r[map.workNature]||"")?"Incident":/chg|change/i.test(r[map.workNature]||"")?"Change":"Request",desc:r[map.desc]||""};
    });

    React.useEffect(()=>{ if(importRows.length) setImportPrev(buildPrev(importRows,importMap)); },[importMap]);
    ...
    const runImport = () => {
      ...
      setTimeout(()=>{
        ...
        const newActs=importRows.map(r=>{
          const raw=importMap.estMins?r[importMap.estMins]:""; const hrs=parseFloat(raw)||8;
          const estMins=hrs<50?Math.round(hrs*60):Math.round(hrs);
          const type=r[importMap.type]||"BAU";
          const nt=/proj/i.test(type)?"Project":/inc/i.test(type)?"Incident":/chg|change/i.test(type)?"Change":/train/i.test(type)?"Training":"BAU";
          const prio=r[importMap.priority]||"Medium";
          const np=/crit/i.test(prio)?"Critical":/high/i.test(prio)?"High":/low/i.test(prio)?"Low":"Medium";
          return {name:(r[importMap.name]||"").trim(),type:nt,cat:r[importMap.cat]||"Maintenance",priority:np,estMins,ticketNo:r[importMap.ticketNo]||"",jira:r[importMap.jira]||r[importMap.ticketNo]||"",nature:/react/i.test(r[importMap.nature]||"")?"Reactive":"Proactive",workNature:/inc/i.test(r[importMap.workNature]||"")?"Incident":/chg|change/i.test(r[importMap.workNature]||"")?"Change":"Request",desc:r[importMap.desc]||"",status:"Active",date:today(),team:user.team};
    ...
    const dlTemplate = () => {
      const csv=["Activity Name,Type,Category,Priority,Est. Hours,Ticket / Ref,JIRA Ref,Nature,Work Nature,Description","Firewall Rule Review,Change,Security & Compliance,High,4,CHG-0001,,Proactive,Change,Review and update firewall rules","Daily Health Checks,BAU,Monitoring & Alerting,Low,1,,,Proactive,Request,Daily server health monitoring","P1 Incident Response,Incident,Maintenance,Critical,8,INC-0001,,Reactive,Incident,Respond to P1 production outage","Cloud Cost Optimisation,Project,Infrastructure Build,Medium,16,REQ-0001,,Proactive,Request,Review and optimise cloud spend"].join("\n");
      const a=document.createElement("a"); a.href=URL.createObjectURL(new Blob([csv],{type:"text/csv"})); a.download="isms_activities_template.csv"; a.click();
    };

    const resetImport = ()=>{ setShowImport(false);setImportStep("upload");setImportFile(null);setImportRows([]);setImportCols([]);setImportMap({});setImportPrev([]);setImportResult(null); };
    ...
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          {isMgr && <button onClick={dlTemplate} style={{display:"flex",alignItems:"center",gap:5,padding:"8px 14px",borderRadius:"var(--radius-md)",background:"var(--surface)",border:"1px solid var(--border)",color:"var(--text-2)",fontWeight:600,fontSize:12,cursor:"pointer"}} onMouseEnter={e=>{e.currentTarget.style.borderColor="var(--brand)";e.currentTarget.style.color="var(--brand)";}} onMouseLeave={e=>{e.currentTarget.style.borderColor="var(--border)";e.currentTarget.style.color="var(--text-2)";}}>⬇ Template</button>}
          {isMgr && <button onClick={()=>setShowImport(true)} style={{display:"flex",alignItems:"center",gap:5,padding:"8px 14px",borderRadius:"var(--radius-md)",background:"var(--surface)",border:"1px solid var(--border)",color:"var(--text-2)",fontWeight:600,fontSize:12,cursor:"pointer"}} onMouseEnter={e=>{e.currentTarget.style.borderColor="var(--brand)";e.currentTarget.style.color="var(--brand)";}} onMouseLeave={e=>{e.currentTarget.style.borderColor="var(--border)";e.currentTarget.style.color="var(--text-2)";}}>📥 Import Excel/CSV</button>}
          {isMgr && <Btn onClick={()=>{setAForm({...bAct,team:user.team});setTarget(null);setModal("act");}}>+ New Activity</Btn>}
        </div>
      </div>
      {isAdmin && <InfoBanner color="#1e40af" bg="var(--brand-light)" border="#bfdbfe" icon="📋">Admin view — read-only. Activities are created and managed by team managers.</InfoBanner>}
      ...
      <div style={{display:"flex",gap:9,flexWrap:"wrap",background:"var(--surface)",border:"1px solid var(--border)",borderRadius:"var(--radius-md)",padding:"11px 14px",marginBottom:14}}>
        {isAdmin && (
          <select value={actF.team} onChange={e=>setActF(f=>({...f,team:e.target.value}))} style={{padding:"6px 10px",borderRadius:7,border:"1px solid var(--border-2)",fontSize:12,background:"var(--surface-2)",fontFamily:"var(--font-sans)"}}>
        ...
        {[["type",["","Project","Incident","Change","BAU","Training","Meeting"],"All Types"],
          ...
          <select key={k} value={actF[k]} onChange={e=>setActF(f=>({...f,[k]:e.target.value}))} style={{padding:"6px 10px",borderRadius:7,border:"1px solid var(--border-2)",fontSize:12,background:"var(--surface-2)",fontFamily:"var(--font-sans)"}}>
        ...
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5,fontFamily:"var(--font-sans)"}}>
            ...
            <tbody>
                ...
                return (
                  <tr key={a.id} className="trow-hover">
                    ...
                    <TD>
                      ...
                      <div style={{fontSize:10,color:"var(--text-4)",marginTop:2,fontFamily:"var(--font-mono)"}}>{a.jira}</div>
                    </TD>
                    {/* 3. Team */}
                    ...
                    {/* 7. Type */}
                    <TD s={{fontSize:12,color:"var(--text-2)"}}>{a.type}</TD>
                    {/* 8. Category */}
                    <TD s={{fontSize:11,color:"var(--text-3)",maxWidth:140,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.cat}</TD>
                    {/* 9. Est. Time */}
                    <TD s={{fontWeight:600,fontFamily:"var(--font-mono)",color:"var(--text-2)"}}>{fmtM(a.estMins)}</TD>
                    {/* 10. Logged */}
                    <TD s={{color:"var(--brand)",fontWeight:700,fontFamily:"var(--font-mono)"}}>{fmtM(logged)}</TD>
                    {/* 11. Progress */}
                    <TD>
                      <div style={{display:"flex",alignItems:"center",gap:7}}>
                        <div style={{width:65,height:6,background:"var(--surface-3)",borderRadius:99,overflow:"hidden"}}>
                          <div style={{height:"100%",width:`${pct}%`,background:pct>90?"#dc2626":pct>60?"#059669":"var(--brand)",borderRadius:99}}/>
                        </div>
                        <span style={{fontSize:11,color:"var(--text-3)",fontWeight:600,fontFamily:"var(--font-mono)",minWidth:28}}>{pct}%</span>
                      </div>
                    </TD>
                    {/* 12. Priority */}
              ...
              {filtActs.length===0 && (
                <tr>
                  <td colSpan={isMgr?14:13} style={{textAlign:"center",padding:50,color:"var(--text-4)",fontSize:13}}>
                    {isMgr?"No activities yet. Click + New Activity or Import Excel/CSV.":"No activities found."}
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="modal-backdrop" onClick={e=>e.target===e.currentTarget&&resetImport()}
          ...
          <div className="modal-content" style={{background:"var(--surface)",borderRadius:"var(--radius-xl)",width:"100%",maxWidth:860,boxShadow:"var(--shadow-xl)",overflow:"hidden",marginBottom:32}}>
            ...
            <div style={{padding:"24px 28px"}}>
              ...
              {importStep==="upload" && (
                <div>
                  <div onDragOver={e=>{e.preventDefault();setDragOver2(true);}} onDragLeave={()=>setDragOver2(false)}
                    ...
                    <div style={{fontSize:15,fontWeight:700,color:"var(--text-1)",marginBottom:6}}>Drop your CSV or Excel-exported CSV here</div>
                    <div style={{fontSize:12,color:"var(--text-3)",marginBottom:14}}>
                      In Excel: <strong>File → Save As → CSV (Comma delimited)</strong> · Tab-separated also works
                    </div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                    ...
                    <div style={{background:"var(--surface-2)",border:"1px solid var(--border)",borderRadius:"var(--radius-md)",padding:"16px 18px"}}>
                      <div style={{fontWeight:700,fontSize:13,color:"var(--text-1)",marginBottom:10}}>💡 How It Works</div>
                      {[["Auto-detect","Column headers are matched by keyword — your column names don't need to be exact."],["Est. Hours","Enter effort in hours. 8 = 1 day, 40 = 1 week. Decimals fine (2.5 = 2h 30m)."],["Type values","Project · Change · BAU · Incident · Training · Meeting"],["Priority","Critical · High · Medium · Low (auto-detected from text)"],["Your team",`All imported activities are added to: ${user.team}`]].map(([t,d])=>(
                      ...
                      <button onClick={dlTemplate} style={{marginTop:10,width:"100%",padding:"9px",borderRadius:"var(--radius-md)",background:"var(--surface)",border:"1.5px solid var(--brand)",color:"var(--brand)",fontWeight:700,fontSize:12,cursor:"pointer"}}>⬇ Download CSV Template</button>
                    </div>
                  </div>
                </div>
              )}
                ...
                <div>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
                    <div>
                      ...
                      <div style={{fontSize:12,color:"var(--text-3)",marginTop:2}}>{importRows.length} rows · {importCols.length} columns detected · Review auto-mapping</div>
                    </div>
                    ...
                    {ACT_FIELDS.map(f=>{
                      ...
                      return (
                        <div key={f.key} style={{background:mapped?"#f0fdf4":"var(--surface-2)",border:`1px solid ${mapped?"#86efac":"var(--border)"}`,borderRadius:"var(--radius-sm)",padding:"10px 12px"}}>
                          ...
                          <select value={importMap[f.key]||""} onChange={e=>setImportMap(p=>({...p,[f.key]:e.target.value||undefined}))}
                  ...
                  {importPrev.length>0 && (
                    <div style={{marginBottom:18,border:"1px solid var(--border)",borderRadius:"var(--radius-md)",overflow:"hidden"}}>
                      <div style={{padding:"10px 16px",background:"var(--surface-2)",borderBottom:"1px solid var(--border)",fontSize:12,fontWeight:700,color:"var(--text-1)"}}>Preview — first {importPrev.length} of {importRows.length} rows</div>
                      <div style={{overflowX:"auto"}}>
                        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                          <thead><tr>{["#","Activity Name","Type","Category","Priority","Est.","Ticket","Nature","Work Type"].map(h=><TH key={h} c={h}/>)}</tr></thead>
                          <tbody>
                            {importPrev.map((r,i)=>(
                              <tr key={i} className="trow-hover" style={{borderBottom:"1px solid var(--surface-3)"}}>
                                ...
                                <TD>{r.name?<span style={{fontWeight:600}}>{r.name}</span>:<span style={{color:"#dc2626",fontSize:11}}>⚠ empty — skip</span>}</TD>
                                <TD><span style={{fontSize:11,background:"var(--brand-light)",color:"var(--brand)",padding:"2px 7px",borderRadius:4,fontWeight:600}}>{r.type}</span></TD>
                                <TD s={{fontSize:11,color:"var(--text-3)"}}>{r.cat}</TD>
                                <TD><PDot p={r.priority}/></TD>
                                <TD s={{fontFamily:"var(--font-mono)",fontWeight:600}}>{fmtM(r.estMins)}</TD>
                                <TD>{r.ticketNo?<code style={{background:"var(--surface-3)",padding:"2px 6px",borderRadius:3,fontSize:11,fontFamily:"var(--font-mono)"}}>{r.ticketNo}</code>:<span style={{color:"var(--text-4)"}}>—</span>}</TD>
                                <TD><NaturePill n={r.nature}/></TD>
                                <TD><WNPill w={r.workNature}/></TD>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div style={{fontSize:12,color:"#d97706"}}>{importRows.filter(r=>!r[importMap.name]?.trim()).length>0?`⚠ ${importRows.filter(r=>!r[importMap.name]?.trim()).length} rows with empty names will be skipped`:""}</div>
                    <button onClick={runImport} disabled={!importMap.name||importStep==="importing"}
              ...
              {importStep==="done" && importResult && (
                <div style={{textAlign:"center",padding:"16px 0"}}>
                  ...
                  <div style={{fontSize:13,color:"var(--text-3)",marginBottom:24}}>Activities added to <strong>{user.team}</strong> team</div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,maxWidth:360,margin:"0 auto 24px"}}>
                    {[{label:"Total Rows",val:importResult.total,c:"var(--brand)",bg:"var(--brand-light)"},{label:"Imported",val:importResult.created,c:"#16a34a",bg:"#f0fdf4"},{label:"Skipped",val:importResult.skipped,c:"var(--text-3)",bg:"var(--surface-2)"}].map(s=>(
        ...
        <Card style={{padding:0,overflow:"hidden"}}>
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5,fontFamily:"var(--font-sans)"}}>
              <thead>
                <tr>
                  <TH c="Date"/><TH c="Member"/><TH c="Activity"/><TH c="Ticket"/>
                  <TH c="Work Type"/><TH c="Nature"/><TH c="Team"/><TH c="Time Spent"/><TH c="Notes"/>
              ...
              <tbody>
                {viewLogs.map(l=>{
                  ...
                  return (
                    <tr key={l.id} className="trow-hover" style={{background:isHighlighted?`${tCol(l.team)}06`:""}}>
                      ...
                      <TD>
                        <div style={{display:"flex",alignItems:"center",gap:7}}>
                          ...
                          <div>
                            ...
                            <div style={{fontSize:10,color:"var(--text-4)"}}>{l.cat}</div>
                          </div>
                        </div>
                      </TD>
                      <TD>
                      ...
                      <TD s={{fontSize:11,color:"var(--text-3)",maxWidth:180}}>{l.notes||"—"}</TD>
                ...
                {viewLogs.length===0&&<tr><td colSpan={isMgrOrAdmin?10:9} style={{textAlign:"center",padding:40,color:"var(--text-4)",fontSize:13}}>
  ...
  return (
    <div style={{display:"flex",height:"100vh",fontFamily:"var(--font-sans)",background:"var(--surface-3)",overflow:"hidden",position:"relative"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
        ...
        <div style={{flexShrink:0,background:"var(--surface)",borderBottom:"1px solid var(--border)",padding:"0 24px",height:56,display:"flex",alignItems:"center",justifyContent:"space-between",boxShadow:"var(--shadow-sm)"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            {isMgr && <Btn sm onClick={()=>{setAForm({...bAct,team:user.team});setTarget(null);setModal("act");}}>+ Activity</Btn>}
            {isMgrOrAdmin && <Btn sm v="sec" onClick={()=>{setLForm(bLog);setModal("log");}}>⏱ Log Time</Btn>}
      ...
      {modal==="log" && (() => {
        ...
        // Step completion tracking for visual progress
        const step1Done = !!lForm.actId;
        const step2Done = step1Done && !!lForm.nature && !!lForm.workNature;
        const step3Done = step2Done; // ticket is optional
        const canSave   = step1Done && lForm.mins > 0;

        const StepHead = ({n, label, done, active}) => (
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:done||active?14:0}}>
            <div style={{
            ...
            <span style={{
              ...
              color: done?"#16a34a": active?"var(--text-1)":"var(--text-4)",
              letterSpacing:"-.2px"
            }}>{label}</span>
            {done && !active && <div style={{flex:1,height:1,background:"#d1fae5",marginLeft:4}}/>}
          </div>
        );
        ...
        return (
          <ModalWrap title={isEdit?"✏️ Edit Time Entry":"⏱ Log Time"} onClose={closeM} wide>
            ...
            {isEdit && (
              <InfoBanner color="#065f46" bg="#f0fdf4" border="#a7f3d0" icon="✅">
                Editing entry for <strong>{target.member}</strong> — adjust any field below
              </InfoBanner>
            )}

            {/* ── STEP 1: SELECT ACTIVITY ── */}
            <div style={{marginBottom:20,paddingBottom:20,borderBottom:"1px solid var(--border)"}}>
              <StepHead n={1} label="Select Activity" done={step1Done} active={!step1Done}/>
              {(step1Done || !isEdit) && (
                <>
                  <Lbl t="Activity *">
                    <select style={{...sS,fontSize:13}} value={lForm.actId} onChange={e=>{
                      setLForm(f=>({...f, actId:e.target.value}));
                    }}>
                      <option value="">— Choose an activity to log time against —</option>
                      {logSrc.map(a=>(
                  ...
                  {selAct && (
                    <div style={{
                      ...
                      <div style={{
                        ...
                        background:`${tCol(selAct.team)}18`,color:tCol(selAct.team),
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:10}}>
                          ...
                          <span style={{fontSize:11,color:"var(--text-4)"}}>Est: {fmtM(selAct.estMins)}</span>
                          <span style={{color:"var(--border)",fontSize:11}}>·</span>
                          <span style={{fontSize:11,color:"var(--brand)",fontWeight:600}}>
                            {fmtM(minsForAct(selAct.id))} logged so far
                          </span>
                        </div>
                        {/* Team progress bar */}
                        <div style={{marginTop:8}}>
                          <div style={{height:4,background:"var(--surface-3)",borderRadius:99,overflow:"hidden"}}>
                            <div style={{
                              ...
                              width:`${Math.min(100,Math.round(minsForAct(selAct.id)/Math.max(selAct.estMins,1)*100))}%`,
                              background:tCol(selAct.team)
                            }}/>
                          </div>
                        </div>
                      </div>
            ...
            <div style={{marginBottom:20,paddingBottom:20,borderBottom:"1px solid var(--border)",opacity:step1Done?1:.45,pointerEvents:step1Done?"all":"none",transition:"opacity .2s"}}>
              <StepHead n={2} label="Classify This Work" done={step2Done} active={step1Done&&!step2Done}/>

              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>

                {/* Nature: Proactive / Reactive */}
                <Lbl t="Nature — How did this work arise?">
                  <div style={{display:"flex",gap:8,marginTop:4}}>
                    {[
                      {val:"Proactive", icon:"⬆", desc:"Planned / scheduled", bg:"#eff6ff", col:"#1d4ed8"},
                      {val:"Reactive",  icon:"⬇", desc:"Unplanned / triggered", bg:"#fff1f2", col:"#be123c"},
                    ].map(n=>(
                      <button key={n.val} onClick={()=>setLForm(f=>({...f,nature:n.val}))}
                        style={{
                          flex:1,padding:"10px 8px",borderRadius:"var(--radius-md)",cursor:"pointer",
                          border:`2px solid ${lForm.nature===n.val?n.col:"var(--border)"}`,
                          background:lForm.nature===n.val?n.bg:"var(--surface)",
                          transition:"all .15s",textAlign:"center"
                        }}>
                        <div style={{fontSize:16,marginBottom:3}}>{n.icon}</div>
                        <div style={{fontWeight:700,fontSize:12,color:lForm.nature===n.val?n.col:"var(--text-2)"}}>{n.val}</div>
                        <div style={{fontSize:10,color:"var(--text-4)",marginTop:2}}>{n.desc}</div>
                      </button>
                    ))}
                  </div>
                </Lbl>

                {/* Work Type: Change / Request / Incident / BAU / Project */}
                <Lbl t="Work Type — What kind of work is this?">
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginTop:4}}>
                    {[
                      {val:"Change",   icon:"🔄", col:"#b45309"},
                      {val:"Request",  icon:"📥", col:"#5b21b6"},
                      {val:"Incident", icon:"🚨", col:"#be123c"},
                      {val:"BAU",      icon:"🔧", col:"#065f46"},
                      {val:"Project",  icon:"📁", col:"#1d4ed8"},
                    ].map(w=>(
                      <button key={w.val} onClick={()=>setLForm(f=>({...f,workNature:w.val}))}
                        style={{
                          padding:"8px 4px",borderRadius:"var(--radius-sm)",cursor:"pointer",
                          border:`2px solid ${lForm.workNature===w.val?w.col:"var(--border)"}`,
                          background:lForm.workNature===w.val?`${w.col}12`:"var(--surface)",
                          transition:"all .15s",textAlign:"center"
                        }}>
                        <div style={{fontSize:14,marginBottom:2}}>{w.icon}</div>
                        <div style={{fontWeight:700,fontSize:11,color:lForm.workNature===w.val?w.col:"var(--text-2)"}}>{w.val}</div>
                      </button>
                    ))}
                  </div>
                </Lbl>
            ...
            {/* ── STEP 3: TICKET REFERENCE ── */}
            <div style={{marginBottom:20,paddingBottom:20,borderBottom:"1px solid var(--border)",opacity:step1Done?1:.45,pointerEvents:step1Done?"all":"none",transition:"opacity .2s"}}>
              <StepHead n={3} label="Ticket Reference" done={!!lForm.ticketRef} active={step1Done&&!lForm.ticketRef}/>

              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                <Lbl t="Ticket / Reference No. (optional)">
                  <input style={{...iS,fontFamily:"var(--font-mono)",fontWeight:600,letterSpacing:".5px"}}
                  ...
                  <div style={{fontSize:11,color:"var(--text-4)",marginTop:5,lineHeight:1.5}}>
                    Enter the specific ticket number for <em>this instance</em> of work — independent of the activity template above.
                  </div>
                </Lbl>

                {/* Summary chip — shows what will be recorded */}
                <div style={{paddingTop:18}}>
                  <div style={{background:"var(--surface-2)",border:"1px solid var(--border)",borderRadius:"var(--radius-md)",padding:"12px 14px"}}>
                    <div style={{fontSize:10,fontWeight:700,color:"var(--text-4)",textTransform:"uppercase",letterSpacing:1,fontFamily:"var(--font-mono)",marginBottom:10}}>Entry Preview</div>
                    <div style={{display:"flex",flexDirection:"column",gap:6}}>
                      {[
                        ["Activity", selAct?.name||"—"],
                        ["Nature",   lForm.nature||"—"],
                        ["Work Type",lForm.workNature||"—"],
                        ["Ticket",   lForm.ticketRef||"(none)"],
                      ].map(([k,v])=>(
                        <div key={k} style={{display:"flex",alignItems:"baseline",gap:6}}>
                          <span style={{fontSize:10,color:"var(--text-4)",fontFamily:"var(--font-mono)",minWidth:70}}>{k}</span>
                          <span style={{fontSize:12,fontWeight:600,color:"var(--text-1)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{v}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {/* ── STEP 4: TIME, DATE, NOTES ── */}
            <div style={{opacity:step1Done?1:.45,pointerEvents:step1Done?"all":"none",transition:"opacity .2s"}}>
              <StepHead n={4} label="Time, Date & Notes" done={false} active={step1Done}/>
                    ...
                    {[[15,"15m"],[30,"30m"],[45,"45m"],[60,"1h"],[90,"1.5h"],[120,"2h"],[180,"3h"],[240,"4h"],[300,"5h"],[480,"8h"]].map(([v,l])=>(
                      <button key={v} onClick={()=>setLForm(f=>({...f,mins:v}))}
                        style={{
                          ...
                          color:+lForm.mins===v?"#fff":"var(--text-2)",
