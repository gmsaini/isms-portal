        ...
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5,fontFamily:"var(--font-sans)"}}>
            ...
            <tbody>
              {filtActs.map(a=>{
                ...
                return (
                  <tr key={a.id} className="trow-hover">
                    ...
                    <TD><div style={{fontWeight:600}}>{a.name}</div><div style={{fontSize:10,color:"var(--text-4)",marginTop:1,fontFamily:"var(--font-mono)"}}>{a.jira}</div></TD>
                    ...
                    <TD s={{fontSize:11,color:"var(--text-3)"}}>{a.cat}</TD>
                    <TD s={{fontWeight:600}}>{fmtM(a.estMins)}</TD>
                    <TD s={{color:"var(--brand)",fontWeight:700}}>{fmtM(logged)}</TD>
                    <TD>
                      <div style={{display:"flex",alignItems:"center",gap:7}}>
                        <div style={{width:65,height:5,background:"var(--surface-3)",borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:`${pct}%`,background:pct>90?"#dc2626":pct>60?"#059669":"var(--brand)",borderRadius:3}}/></div>
              ...
              {filtActs.length===0&&<tr><td colSpan={isMgr?14:13} style={{textAlign:"center",padding:50,color:"var(--text-4)",fontSize:13}}>{isMgr?"No activities yet. Click + New Activity.":"No activities found."}</td></tr>}
            </tbody>
          </table>
        </div>
      </Card>
          ...
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5,fontFamily:"var(--font-sans)"}}>
              ...
              <tbody>
                {viewLogs.map(l=>{
                  ...
                  return (
                    <tr key={l.id} className="trow-hover" style={{background:isHighlighted?`${tCol(l.team)}06`:""}}>
                      ...
                      <TD s={{fontSize:11,color:"var(--text-3)",maxWidth:200}}>{l.notes||"—"}</TD>
                ...
                {viewLogs.length===0&&<tr><td colSpan={isAdmin?8:isMgr?8:9} style={{textAlign:"center",padding:40,color:"var(--text-4)",fontSize:13}}>
      ...
      <div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:18}}>
          <div>
            ...
            <div style={{fontSize:12,color:"var(--text-3)",marginTop:3}}>{isAdmin?"All users across all teams — click ✏️ to edit any member":"Your team members"}</div>
          </div>
          <div style={{display:"flex",gap:9,alignItems:"center"}}>
            {isAdmin && <input placeholder="🔍 Search members…" value={mSearch} onChange={e=>setMSearch(e.target.value)} style={{padding:"7px 11px",borderRadius:"var(--radius-sm)",border:"1px solid var(--border-2)",fontSize:12,width:200}}/>}
              ...
              {g.members.map(m=>{
                ...
                return (
                  <Card key={m.id} style={{padding:16,position:"relative"}}>
                    {canEdit && (
                      <div style={{position:"absolute",top:10,right:10,display:"flex",gap:5}}>
                        <button onClick={()=>openEdit(m)} style={{width:26,height:26,borderRadius:6,background:"var(--surface-3)",border:"1px solid var(--border)",cursor:"pointer",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center"}} title="Edit">✏️</button>
                        {m.id!==user.id && <button onClick={()=>setMConfirm(m.id)} style={{width:26,height:26,borderRadius:6,background:"#fef2f2",border:"1px solid #fecaca",cursor:"pointer",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center"}} title="Remove">🗑</button>}
                      </div>
                    )}
                    ...
                    <div style={{fontWeight:700,fontSize:13,color:"var(--text-1)"}}>{m.name}</div>
                    <div style={{fontSize:11,color:"var(--text-3)",marginBottom:8}}>{m.title||m.role}</div>
        ...
        {mConfirm && (()=>{
          ...
          return (
            <ModalWrap title="🗑 Remove Member" onClose={()=>setMConfirm(null)}>
              <div style={{textAlign:"center",padding:"8px 0 16px"}}>
                <div style={{fontSize:40,marginBottom:12}}>⚠️</div>
                <div style={{fontSize:14,fontWeight:700,color:"var(--text-1)",marginBottom:6}}>Remove {m?.name}?</div>
                <div style={{fontSize:12,color:"var(--text-3)"}}>This will remove the member from ISMS. Their existing time log entries will be preserved.</div>
              </div>
              <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
                <button onClick={()=>setMConfirm(null)} style={{padding:"8px 20px",borderRadius:"var(--radius-sm)",background:"var(--surface)",border:"1px solid var(--border-2)",fontWeight:600,fontSize:13,cursor:"pointer"}}>Cancel</button>
                <button onClick={()=>deleteMember(mConfirm)} style={{padding:"8px 20px",borderRadius:"var(--radius-sm)",background:"#dc2626",color:"#fff",border:"none",fontWeight:700,fontSize:13,cursor:"pointer"}}>Remove</button>
              </div>
            </ModalWrap>
          );
        })()}
                    ...
                    return (
                      <tr key={a.id} style={{background:rowBg,borderBottom:"1px solid var(--surface-3)"}}
                        ...
                        <td style={{padding:"10px 10px",whiteSpace:"nowrap"}}>
                          <code style={{background:"var(--surface-3)",padding:"3px 7px",borderRadius:5,
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
        ...
        <div style={{flexShrink:0,background:"var(--surface)",borderBottom:"1px solid var(--border)",padding:"0 24px",height:56,display:"flex",alignItems:"center",justifyContent:"space-between",boxShadow:"var(--shadow-sm)"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            {isMgr && <Btn sm v="sec" onClick={()=>{setLForm(bLog);setModal("log");}}>⏱ Log Time</Btn>}
          </div>
        </div>
              ...
              <Lbl t={`Time Spent * = ${fmtM(+lForm.mins)}`}>
                ...
                <div style={{display:"flex",gap:5,marginTop:6,flexWrap:"wrap"}}>
                  {[[15,"15m"],[30,"30m"],[60,"1h"],[90,"1.5h"],[120,"2h"],[180,"3h"],[240,"4h"],[480,"8h"]].map(([v,l])=>(
                    <button key={v} onClick={()=>setLForm(f=>({...f,mins:v}))} style={{padding:"3px 8px",borderRadius:"var(--radius-xl)",border:`1.5px solid ${+lForm.mins===v?"var(--brand)":"var(--border-2)"}`,background:+lForm.mins===v?"var(--brand)":"var(--surface)",color:+lForm.mins===v?"#fff":"var(--text-2)",fontSize:11,fontWeight:600,cursor:"pointer"}}>{l}</button>
                  ))}
                </div>
              </Lbl>
