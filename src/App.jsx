...
function PortalApp({initialUser, allUsersInit}) {
  ...
  const [acts,       setActs]       = useState(()=>{
    try {
      ...
      if (!s) return [...SEED_ACT, ...SEED_MI];
      const saved = JSON.parse(s);
      // Always ensure seed data is present even if localStorage was populated before seeds existed
      const savedIds = new Set(saved.map(a=>a.id));
      const missingSeed = [...SEED_ACT,...SEED_MI].filter(a=>!savedIds.has(a.id));
      return [...missingSeed, ...saved];
    } catch(e) { return [...SEED_ACT,...SEED_MI]; }
  });
  ...
  const runActImport = () => {
    ...
    setTimeout(()=>{
      ...
      setActs(p=>{
        const startN = p.filter(a=>!a.isMajorIncident).reduce((max,a)=>{
  ...
  const loggable  = useMemo(()=>{
    if(isAdmin) return acts.filter(a=>a.isMajorIncident ? a.status==="Open" : a.status==="Active");
    return user?.team
      ? acts.filter(a=>a.isMajorIncident
          ? a.status==="Open"
          : a.status==="Active" && a.team===user.team)
      : [];
  },[acts,user,isAdmin]);
  ...
  const LIVE_CAPACITY = useMemo(()=>{
    ...
    return base.map(c=>({
      ...c,
      utilMins: logs.filter(l=>l.team===c.team&&!acts.find(a=>a.id===l.actId)?.isMajorIncident)
                    .reduce((s,l)=>s+l.mins, 0),
    }));
  }, [allUsers, logs, acts]);
  ...
  const saveAct = () => {
    ...
    if(target){
      ...
      const nextN = acts.filter(a=>!a.isMajorIncident).reduce((max,a)=>{
        const n = parseInt((a.id||"").replace("ACT-",""),10)||0;
        return n>max?n:max;
      },0)+1;
  ...
  const saveLog = () => {
    ...
    if(target){
      ...
      setLogs(p=>[{id:uid("TL"),date:lForm.date,userId:user.id,member:user.name,team:act.isMajorIncident?user.team:act.team,actId:lForm.actId,activity:act.name,type:act.type,cat,mins:+lForm.mins,notes:lForm.notes,ticketRef,nature,workType,workNature,priority},...p]);
  ...
  const reIdActivities = () => {
    const mis     = acts.filter(a => a.isMajorIncident);
    const regular = [...acts.filter(a => !a.isMajorIncident)]
    ...
    setActs(newActs);
    setLogs(newLogs);
    showToast(`${regular.length} activities re-IDed alphabetically`);
  };
  ...
  const PageAbout = () => {
    ...
    const L = useMemo(() => {
      ...
      return {
        totalActs:      acts.filter(a=>!a.isMajorIncident).length,
        activeActs:     acts.filter(a=>a.status==="Active"&&!a.isMajorIncident).length,
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            <Lbl t="Task Type">
              <select style={sS} value={aForm.type} onChange={e=>setAForm(f=>({...f,type:e.target.value}))}>
                {["BAU","Project","Change","Incident","Training","Meeting"].map(o=><option key={o}>{o}</option>)}
              </select>
            </Lbl>
