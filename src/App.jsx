...
function PortalApp({initialUser, allUsersInit}) {
  ...
  const [acts,       setActs]       = useState(()=>{
    try { const s=localStorage.getItem('isms_acts'); return s?JSON.parse(s):SEED_ACT; } catch(e){ return SEED_ACT; }
  });
              ...
              {filtActs.map(a=>{
                ...
                return (
                  <tr key={a.id} className="trow-hover">
                    ...
                    {isMgr && <TD><div style={{display:"flex",gap:5}}><Btn sm v="sec" onClick={()=>openEditA(a)}>✏️</Btn><Btn sm v="dan" onClick={()=>delAct(a.id)}>🗑</Btn></div></TD>}
                  </tr>
                );
              })}
              {filtActs.length===0 && (
                <tr><td colSpan={isMgr?11:10} style={{textAlign:"center",padding:50,color:"var(--text-4)",fontSize:13}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                    ...
                    <div style={{background:"var(--surface-2)",border:"1px solid var(--border)",borderRadius:"var(--radius-md)",padding:"16px 18px"}}>
                      ...
                      {[
                        ...
                        ["Your team",`All rows will be added to: ${user.team||"your team"}`],
                      ].map(([t,d])=>(
