...
function PortalApp({initialUser, allUsersInit}) {
  ...
  const PageDashboard = () => {
    ...
    const ColumnChart = ({data, height=140, onClick}) => {
      if(!data||!data.length) return null;
      const max = Math.max(...data.map(d=>d.val),1);
      const W=100, padX=4, barW=Math.min(14,(W-padX*2)/data.length*.7);
      const gap=(W-padX*2-barW*data.length)/(data.length);
      return (
        <svg viewBox={`0 0 ${W} ${height}`} style={{width:"100%",height,overflow:"visible"}}>
          <defs>
            {data.map((d,i)=>(
              <linearGradient key={i} id={`cg${i}`} x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={d.color||"#1a56db"} stopOpacity="1"/>
                <stop offset="100%" stopColor={d.color||"#1a56db"} stopOpacity=".6"/>
              </linearGradient>
            ))}
          </defs>
          {/* Grid lines */}
          {[25,50,75,100].map(p=>{
            const y=height-20-(p/100*(height-28));
            return <line key={p} x1={padX} y1={y} x2={W-padX} y2={y} stroke="#f1f5f9" strokeWidth=".8"/>;
          })}
          {data.map((d,i)=>{
            const bh = Math.max(3,(d.val/max)*(height-28));
            const x = padX + i*(barW+gap) + gap/2;
            const y = height-20-bh;
            const isHovered = false;
            return (
              <g key={i} style={{cursor:onClick?"pointer":"default"}}
                onClick={()=>onClick&&onClick(d)}>
                {/* Shadow bar */}
                <rect x={x+1.5} y={y+2} width={barW} height={bh} rx="3" fill={d.color||"#1a56db"} opacity=".12"/>
                {/* Main bar */}
                <rect x={x} y={y} width={barW} height={bh} rx="3" fill={`url(#cg${i})`}/>
                {/* Value on top */}
                <text x={x+barW/2} y={y-3} textAnchor="middle" fontSize="5.5" fontWeight="700" fill="#374151">{d.val}</text>
                {/* Label below */}
                <text x={x+barW/2} y={height-6} textAnchor="middle" fontSize="5" fill="#94a3b8"
                  style={{fontFamily:"'DM Sans',sans-serif"}}>{d.label}</text>
              </g>
            );
          })}
        </svg>
    ...
    // ── Lollipop Chart (SVG) ────────────────────────────────────────
    const LollipopChart = ({data, onClick}) => {
      if(!data||!data.length) return null;
      const max = Math.max(...data.map(d=>d.val),1);
      const rowH = 32;
      ...
      return (
        <svg viewBox={`0 0 100 ${H}`} style={{width:"100%",height:H}}>
          {data.map((d,i)=>{
            const barW = Math.max(2,(d.val/max)*68);
            const y = i*rowH + rowH/2;
            const col = d.color||"#1a56db";
            return (
              <g key={i} style={{cursor:onClick?"pointer":"default"}} onClick={()=>onClick&&onClick(d)}>
                {/* Track */}
                <line x1={28} y1={y} x2={96} y2={y} stroke="#f1f5f9" strokeWidth="2" strokeLinecap="round"/>
                {/* Stem */}
                <line x1={28} y1={y} x2={28+barW} y2={y} stroke={col} strokeWidth="2.5" strokeLinecap="round"
                  style={{filter:`drop-shadow(0 1px 2px ${col}55)`}}/>
                {/* Dot */}
                <circle cx={28+barW} cy={y} r="4.5" fill={col} style={{filter:`drop-shadow(0 1px 3px ${col}88)`}}/>
                <circle cx={28+barW} cy={y} r="2" fill="#fff"/>
                {/* Label */}
                <text x={26} y={y+1} textAnchor="end" fontSize="5.5" fontWeight="600" fill="#475569">{d.label}</text>
                {/* Value */}
                <text x={28+barW+7} y={y+1} textAnchor="start" fontSize="5.5" fontWeight="800" fill={col}>{d.val}</text>
              </g>
            );
          })}
        </svg>
            ...
            return (
              <div key={i} onClick={()=>onClick&&onClick(d)}
                style={{display:"flex",alignItems:"center",gap:10,cursor:onClick?"pointer":"default",
                  padding:"6px 8px",borderRadius:9,transition:"background .12s"}}
                onMouseEnter={e=>e.currentTarget.style.background="#f0f9ff"}
                onMouseLeave={e=>e.currentTarget.style.background=""}>
                {/* Mini ring */}
                <svg width="36" height="36" viewBox="0 0 36 36" style={{flexShrink:0}}>
                  <circle cx="18" cy="18" r={R} fill="none" stroke="#f1f5f9" strokeWidth="4"/>
                  <circle cx="18" cy="18" r={R} fill="none" stroke={col} strokeWidth="4"
                    strokeDasharray={`${dash} ${circ}`} strokeLinecap="round"
                    transform="rotate(-90 18 18)" style={{transition:"stroke-dasharray .6s ease"}}/>
                  <text x="18" y="19" textAnchor="middle" fontSize="7" fontWeight="800" fill={col}
                    dominantBaseline="middle">{i+1}</text>
                </svg>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:11.5,fontWeight:700,color:"#0f172a",overflow:"hidden",
                    textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{d.label}</div>
                  {d.sub && <div style={{fontSize:10,color:"#9ca3af",marginTop:1}}>{d.sub}</div>}
                </div>
                <div style={{textAlign:"right",flexShrink:0}}>
                  <div style={{fontSize:15,fontWeight:800,color:col}}>{d.val}</div>
                  {d.unit && <div style={{fontSize:9,color:"#9ca3af"}}>{d.unit}</div>}
                </div>
              </div>
            );
          })}
    ...
    const PieChart = ({data, size=160, onClick}) => {
      ...
      return (
        <svg viewBox="-20 -20 140 140" style={{width:size,height:size,flexShrink:0}}>
          <defs>
            {slices.map((s,i)=>(
              <filter key={i} id={`ps${i}`} x="-20%" y="-20%" width="140%" height="140%">
                <feDropShadow dx="0" dy="1" stdDeviation="2" floodColor={s.color} floodOpacity=".3"/>
              </filter>
            ))}
          </defs>
          {slices.map((s,i)=>(
            <g key={i} style={{cursor:onClick?"pointer":"default"}} onClick={()=>onClick&&onClick(s)}>
              <path d={s.path} fill={s.color} filter={`url(#ps${i})`}
                style={{transition:"transform .15s"}}
                onMouseEnter={e=>{e.currentTarget.style.transform=`translate(${Math.cos(s.mid)*3}px,${Math.sin(s.mid)*3}px)`;}}
                onMouseLeave={e=>{e.currentTarget.style.transform="";}}/>
              {s.pct>=8 && (
                <text x={cx+(R*.58)*Math.cos((s.mid-Math.PI/2+Math.PI/2))} y={cy+(R*.58)*Math.sin((s.mid-Math.PI/2+Math.PI/2))}
                  textAnchor="middle" dominantBaseline="middle" fontSize="6.5" fontWeight="800" fill="#fff">
                  {s.pct}%
                </text>
              )}
            </g>
            ...
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:12}}>
              ...
              <div style={{position:"relative",cursor:"pointer",display:"flex",justifyContent:"center"}} onClick={()=>goPage("activities")}>
                {(()=>{
                  ...
                  return (
                    <svg viewBox="0 0 100 60" style={{width:160,height:96}}>
                      <path d={`M ${cx-R} ${cy} A ${R} ${R} 0 0 1 ${cx+R} ${cy}`} fill="none" stroke="#f1f5f9" strokeWidth={sw}/>
                      {arcs.map((a,i)=>(
                        <path key={i} d={a.path} fill="none" stroke={a.color} strokeWidth={sw}
                          strokeLinecap="butt" style={{filter:`drop-shadow(0 1px 2px ${a.color}55)`}}/>
                      ))}
                      <text x={cx} y={cy-8} textAnchor="middle" fontSize="14" fontWeight="800" fill="#0f172a">{dashActs.length}</text>
                      <text x={cx} y={cy+4} textAnchor="middle" fontSize="6.5" fill="#9ca3af">activities</text>
                    </svg>
                  );
                })()}
              </div>
