...
function PortalApp({initialUser, allUsersInit}) {
  ...
  // forms
  const bAct = {name:"",team:"",type:"BAU",cat:"Infrastructure Build",estMins:480,priority:"Medium",status:"Active",taskSize:"",complexity:"",level:"",desc:"",progress:0};
  ...
  const openEditA = a => { setTarget(a); setAForm({name:a.name,team:a.team,type:a.type,cat:a.cat,estMins:a.estMins,priority:a.priority,status:a.status,taskSize:a.taskSize||"",complexity:a.complexity||"",level:a.level||"",desc:a.desc,progress:a.progress||0}); setModal("act"); };
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5,fontFamily:"var(--font-sans)"}}>
            <thead>
              <tr>
                ...
                <TH c="Priority"/><TH c="Status"/><TH c="Time Logged"/>
            ...
            <tbody>
              {filtActs.map(a=>{
                ...
                return (
                  <tr key={a.id} className="trow-hover">
                    ...
                    <TD>
                      {(()=>{
                        const c={Critical:"#dc2626",High:"#d97706",Medium:"#2563eb",Low:"var(--text-4)"}[a.priority]||"var(--text-4)";
                        return <span style={{fontSize:11,fontWeight:700,color:c}}>{a.priority||"—"}</span>;
                      })()}
                    </TD>
              ...
              {filtActs.length===0 && (
                <tr><td colSpan={isMgrOrAdmin?12:11} style={{textAlign:"center",padding:50,color:"var(--text-4)",fontSize:13}}>
      ...
      {modal==="act" && (
        <ModalWrap title={target?"✏️ Edit Activity":"📋 New Activity"} onClose={closeM} wide>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            {/* Priority */}
            <Lbl t="Priority">
              <select style={sS} value={aForm.priority} onChange={e=>setAForm(f=>({...f,priority:e.target.value}))}>
            ...
            {/* Status */}
            <Lbl t="Status">
              <select style={sS} value={aForm.status} onChange={e=>setAForm(f=>({...f,status:e.target.value}))}>
                {["Active","Pending","Done"].map(o=><option key={o}>{o}</option>)}
              </select>
            </Lbl>
