...
function PortalApp({initialUser, allUsersInit}) {
  const [allUsers,   setAllUsers]   = useState(allUsersInit||SEED_USERS);
  ...
  const PageUserMgmt = () => {
    ...
    // ── User CRUD ────────────────────────────────────────────────────
    const openAddUser = () => {
      setUForm({name:"",title:"",email:"",team:isMgr?user.team:"",role:"member",status:"Active",costPerHour:0});
    ...
    const saveUser = () => {
      ...
      if(uTarget){
        ...
        const nu = {id:uid("USR"),name:uForm.name.trim(),title:uForm.title,email:uForm.email,team:uForm.team||null,role:uForm.role,status:uForm.status,costPerHour:uForm.costPerHour||0};
        setAllUsers(p=>[...p,nu]);
  ...
  return (
    <div style={{display:"flex",height:"100vh",fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#f0f4f8",overflow:"hidden",position:"relative"}}>
      ...
      <div style={{width:236,minWidth:236,height:"100vh",background:"#ffffff",borderRight:"1px solid #eaeef4",display:"flex",flexDirection:"column",overflow:"visible",flexShrink:0,boxShadow:"2px 0 16px rgba(15,23,42,.05)",zIndex:100,fontFamily:"'DM Sans',sans-serif"}}>
        ...
        {/* ── Sticky Sign Out at bottom of sidebar ── */}
        <div style={{flexShrink:0,padding:"10px 12px",borderTop:"1px solid #eaeef4"}}>
          <div onClick={handleLogout}
            style={{display:"flex",alignItems:"center",gap:9,padding:"9px 12px",borderRadius:9,cursor:"pointer",
              background:"#fff5f5",border:"1px solid #fecaca",transition:"all .15s"}}
            onMouseEnter={e=>{e.currentTarget.style.background="#fee2e2";e.currentTarget.style.borderColor="#fca5a5";}}
            onMouseLeave={e=>{e.currentTarget.style.background="#fff5f5";e.currentTarget.style.borderColor="#fecaca";}}>
            <span style={{fontSize:15}}>🚪</span>
            <span style={{fontSize:12,fontWeight:700,color:"#dc2626",flex:1}}>Sign Out</span>
...
export default function App() {
  ...
  const [allUsersForLogin, setAllUsersForLogin] = useState(SEED_USERS);
