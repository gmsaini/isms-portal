  ...
  if(typeof document !== "undefined") {
    ...
    s.textContent = `
      *, *::before, *::after { box-sizing: border-box; }
      ...
      body { font-family: 'DM Sans', system-ui, sans-serif; background: #f0f4f8; color: #1e293b; -webkit-font-smoothing: antialiased; }
      ...
      .page-enter { animation: fadeSlideIn .25s cubic-bezier(.4,0,.2,1) both; }
      .card-hover { transition: transform .2s cubic-bezier(.4,0,.2,1), box-shadow .2s ease; }
      .card-hover:hover { transform: translateY(-3px); box-shadow: 0 12px 32px rgba(15,23,42,.12) !important; }
      .row-hover:hover { background: #f5f8ff !important; }
      .btn-primary { transition: filter .15s, transform .12s, box-shadow .15s; }
      .btn-primary:hover { filter: brightness(1.08); transform: translateY(-1px); box-shadow: 0 6px 16px rgba(26,86,219,.3) !important; }
      ...
      input:focus, select:focus, textarea:focus { outline: none; border-color: #1a56db !important; box-shadow: 0 0 0 3px rgba(26,86,219,.12) !important; }
    `;
...
const Card = ({children,style={},onClick,onMouseEnter,onMouseLeave,className=""}) => (
  <div onClick={onClick} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} className={className}
    style={{background:"#ffffff",border:"1px solid #e8edf5",borderRadius:14,padding:20,
      boxShadow:"0 1px 3px rgba(15,23,42,.04),0 4px 16px rgba(15,23,42,.06)",...style}}>{children}</div>
);
...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if(this.state.hasError) {
      return (
        <div style={{padding:40,maxWidth:600,margin:"60px auto",fontFamily:"'DM Sans',system-ui,sans-serif"}}>
          <div style={{background:"#fef2f2",border:"1px solid #fca5a5",borderRadius:14,padding:"28px 32px"}}>
            ...
            <button onClick={()=>window.location.reload()}
              style={{padding:"9px 22px",borderRadius:9,background:"#dc2626",color:"#fff",border:"none",fontWeight:700,fontSize:13,cursor:"pointer"}}>
              🔄 Reload Page
            </button>
          </div>
        </div>
...
function LoginPage({allUsers, onLogin}) {
  ...
  const handleLogin = () => {
    ...
    setTimeout(()=>{
      console.log("🔍 Login attempt:", cred.username, "Password length:", cred.password.length);
      console.log("📋 Available users:", allUsers.map(u => ({name: u.name, username: u.username, email: u.email, hasPassword: !!u.password})));
...
function PortalApp({initialUser, allUsersInit}) {
  ...
  const PageDashboard = () => {
    ...
    const BarChart = ({data, unit="", onClick}) => {
      if(!data||!data.length) return <div style={{color:"#9ca3af",fontSize:12,padding:"20px 0",textAlign:"center"}}>No data</div>;
              ...
              <div key={d.label||i} onClick={()=>onClick&&onClick(d)}
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                  <div style={{display:"flex",alignItems:"center",gap:7,minWidth:0}}>
                    ...
                    <div style={{width:8,height:8,borderRadius:"50%",background:d.color||"#1a56db",flexShrink:0}}/>
                    <span style={{fontSize:12,fontWeight:600,color:"#374151",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:130}}>{d.label}</span>
                  </div>
                  <span style={{fontSize:12,fontWeight:800,color:d.color||"#1a56db",flexShrink:0,marginLeft:8}}>{d.val}{unit}</span>
                </div>
                <div style={{height:8,background:"#f1f5f9",borderRadius:6,overflow:"hidden"}}>
                  <div style={{
                    height:"100%",
                    width:`${pct}%`,
                    background:`linear-gradient(90deg,${d.color||"#1a56db"},${d.color||"#1a56db"}bb)`,
                    borderRadius:6,
    ...
    const SvgDonut = ({segments, size=120, label, sublabel}) => {
      ...
      return (
        <svg viewBox={`0 0 ${VW} ${VW}`} style={{width:size,height:size,display:"block",flexShrink:0}}>
          ...
          {label && <text x={cx} y={cy-8} textAnchor="middle" fontSize="30" fontWeight="800" fill="#0f172a">{label}</text>}
          {sublabel && <text x={cx} y={cy+18} textAnchor="middle" fontSize="16" fill="#9ca3af">{sublabel}</text>}
        </svg>
      );
    };
    ...
    const ColumnChart = ({data, height=180, unit="", onClick}) => {
      if(!data||!data.length) return <div style={{color:"#9ca3af",fontSize:12,padding:20,textAlign:"center"}}>No data</div>;
      const max = Math.max(...data.map(d=>d.val), 1);
          ...
          <defs>
            {data.map((d,i)=>(
              <linearGradient key={i} id={`vcg_${i}`} x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={d.color||"#1a56db"}/>
                <stop offset="100%" stopColor={d.color||"#1a56db"} stopOpacity=".55"/>
              </linearGradient>
            ))}
          </defs>
          ...
          {[0,25,50,75,100].map(p=>{
            ...
            return (
              <g key={p}>
                ...
                {p>0 && <text x={padL-4} y={y+4} textAnchor="end" fontSize="18" fill="#94a3b8">{Math.round(max*p/100)}{unit}</text>}
              </g>
            );
          })}
          {data.map((d,i)=>{
            ...
            const col = d.color||"#1a56db";
            return (
              <g key={i} style={{cursor:onClick?"pointer":"default"}} onClick={()=>onClick&&onClick(d)}>
                {/* Bar shadow */}
                <rect x={x+3} y={y+4} width={barW} height={bh} rx="6" fill={col} opacity=".15"/>
                {/* Bar */}
                <rect x={x} y={y} width={barW} height={bh} rx="6" fill={`url(#vcg_${i})`}/>
                {/* Value label on top */}
                <text x={cx} y={y-10} textAnchor="middle" fontSize="20" fontWeight="700" fill="#1e293b">{d.val}{unit}</text>
                {/* X axis label */}
                <text x={cx} y={VH-10} textAnchor="middle" fontSize="18" fontWeight="600" fill="#64748b">{d.label}</text>
              </g>
            );
          })}
        </svg>
        ...
        <svg viewBox={`0 0 ${VW} ${VH}`} style={{width:"100%",height:Math.max(120,data.length*42),display:"block"}} preserveAspectRatio="xMidYMid meet">
          {data.map((d,i)=>{
            ...
            return (
              <g key={i} style={{cursor:onClick?"pointer":"default"}} onClick={()=>onClick&&onClick(d)}>
                {/* Track */}
                <line x1={padL} y1={y} x2={VW-padR} y2={y} stroke="#e8edf5" strokeWidth="5" strokeLinecap="round"/>
                {/* Filled stem */}
                ...
                {/* Label — left aligned */}
                <text x={padL-12} y={y+1} textAnchor="end" fontSize="19" fontWeight="600" fill="#374151" dominantBaseline="middle">{d.label}</text>
                {/* Value — right of dot */}
            ...
            return (
              <div key={i} onClick={()=>onClick&&onClick(d)}
                style={{display:"flex",alignItems:"center",gap:12,
                  padding:"8px 10px",borderRadius:10,cursor:onClick?"pointer":"default",
                  transition:"background .12s",background:"#f8fafc"}}
                onMouseEnter={e=>{e.currentTarget.style.background="#eff6ff";}}
                onMouseLeave={e=>{e.currentTarget.style.background="#f8fafc";}}>
                {/* Ring */}
                <svg width="46" height="46" viewBox="0 0 46 46" style={{flexShrink:0}}>
                  <circle cx="23" cy="23" r={R} fill="none" stroke="#e8edf5" strokeWidth="5"/>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:12,fontWeight:700,color:"#0f172a",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{d.label}</div>
                  {d.sub && <div style={{fontSize:10,color:"#9ca3af",marginTop:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{d.sub}</div>}
                </div>
                <div style={{textAlign:"right",flexShrink:0}}>
                  <div style={{fontSize:17,fontWeight:800,color:col,lineHeight:1}}>{d.val}</div>
                  {d.unit && <div style={{fontSize:9,color:"#9ca3af",marginTop:1}}>{d.unit}</div>}
                </div>
              </div>
            );
          })}
      ...
      <div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:12,marginBottom:20}}>
          {[
            ...
            <div key={s.label} onClick={()=>s.drill?s.drill():goPage(s.nav)}
              style={{background:"#fff",borderRadius:14,padding:"14px 16px",cursor:"pointer",
                border:`1px solid #e8edf5`,borderLeft:`3px solid ${s.c}`,
                boxShadow:"0 1px 3px rgba(15,23,42,.04),0 4px 16px rgba(15,23,42,.05)",
                transition:"all .2s cubic-bezier(.4,0,.2,1)",overflow:"hidden"}}
              onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-3px)";e.currentTarget.style.boxShadow=`0 8px 28px rgba(15,23,42,.12)`;e.currentTarget.style.borderColor=s.c;}}
              onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="0 1px 3px rgba(15,23,42,.04),0 4px 16px rgba(15,23,42,.05)";e.currentTarget.style.borderColor="#e8edf5";e.currentTarget.style.borderLeftColor=s.c;}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                ...
                <div style={{textAlign:"right"}}>
                  <div style={{fontSize:9,fontWeight:700,color:"#94a3b8",textTransform:"uppercase",letterSpacing:1.2}}>{s.label}</div>
                  <div style={{fontSize:22,fontWeight:800,color:s.c,lineHeight:1.1,letterSpacing:-.5,marginTop:1}}>{s.val}</div>
                </div>
              </div>
              <SvgSparkline points={s.spark} color={s.c} height={30}/>
              <div style={{fontSize:10.5,color:"#94a3b8",marginTop:5,fontWeight:500}}>{s.sub}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1.4fr 1fr 1fr",gap:14,marginBottom:14}}>
          ...
          <Card className="card-hover" style={{cursor:"pointer"}} onClick={()=>goPage("bandwidth")}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              <div>
                <div style={{fontWeight:700,fontSize:13,color:"#0f172a"}}>Team Utilization</div>
                <div style={{fontSize:11,color:"#9ca3af",marginTop:1}}>% of net capacity used</div>
              </div>
            ...
            <div style={{display:"flex",justifyContent:"center",gap:16,marginTop:8}}>
              {[["#059669","Good (<70%)"],["#d97706","Medium"],["#dc2626","High (>85%)"]].map(([c,l])=>(
          ...
          <Card>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
              <div>
                <div style={{fontWeight:700,fontSize:13,color:"#0f172a",marginBottom:2}}>Activity Status</div>
                <div style={{fontSize:11,color:"#9ca3af"}}>Breakdown by status</div>
              </div>
              <span style={{fontSize:10,color:"#9ca3af"}}>click to drill in</span>
            </div>
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:12}}>
              {/* Semi-circle gauge style */}
              <div style={{position:"relative",cursor:"pointer",display:"flex",justifyContent:"center"}} onClick={()=>goPage("activities")}>
                {(()=>{
                  const valid=actByStat.filter(s=>s.val>0);
                  const total=valid.reduce((s,d)=>s+d.val,0)||1;
                  let angle=180;
                  // Large coordinate space: 400 wide, 220 tall
                  const VW=400, VH=220, cx=200, cy=200, R=155, sw=36;
                  const arcs=valid.map(d=>{
                    const sweep=Math.max(0.5,(d.val/total)*180);
                    const r1=angle*Math.PI/180, r2=(angle+sweep)*Math.PI/180;
                    const x1=cx+R*Math.cos(r1), y1=cy+R*Math.sin(r1);
                    const x2=cx+R*Math.cos(r2), y2=cy+R*Math.sin(r2);
                  ...
                  return (
                    <svg viewBox={`0 0 ${VW} ${VH}`} style={{width:"100%",maxWidth:220,height:120}}>
                      {/* Track */}
                      <path d={`M ${cx-R} ${cy} A ${R} ${R} 0 0 1 ${cx+R} ${cy}`} fill="none" stroke="#f1f5f9" strokeWidth={sw}/>
                      {arcs.map((a,i)=>(
                        <path key={i} d={a.path} fill="none" stroke={a.color} strokeWidth={sw}
                          strokeLinecap="butt" style={{filter:`drop-shadow(0 2px 4px ${a.color}66)`}}/>
                      ))}
                      <text x={cx} y={cy-22} textAnchor="middle" fontSize="54" fontWeight="800" fill="#0f172a">{dashActs.length}</text>
                      <text x={cx} y={cy+12} textAnchor="middle" fontSize="22" fill="#94a3b8">activities</text>
                    </svg>
                  );
                })()}
              </div>
              <div style={{width:"100%",display:"flex",flexDirection:"column",gap:5}}>
                {actByStat.filter(s=>s.val>0).map(s=>(
                  <div key={s.label} onClick={()=>s.val>0&&setDashDrill({type:"acts",label:`${s.label} Activities`,data:dashActs.filter(a=>a.status===s.label)})}
                    style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",borderRadius:8,cursor:s.val>0?"pointer":"default",transition:"all .12s",background:"#f8fafc"}}
                    onMouseEnter={e=>{if(s.val>0){e.currentTarget.style.background=`${s.color}12`;e.currentTarget.style.transform="translateX(2px)";}}}
                    onMouseLeave={e=>{e.currentTarget.style.background="#f8fafc";e.currentTarget.style.transform="";}}>
                    <div style={{width:10,height:10,borderRadius:"50%",background:s.color,flexShrink:0}}/>
                    <span style={{fontSize:12,color:"#374151",fontWeight:500,flex:1}}>{s.label}</span>
                    <span style={{fontSize:14,fontWeight:800,color:s.color}}>{s.val}</span>
                    <span style={{fontSize:10,color:"#9ca3af",minWidth:30,textAlign:"right"}}>{Math.round(s.val/Math.max(dashActs.length,1)*100)}%</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <Card style={{cursor:"pointer"}} onClick={()=>goPage("timelog")}
              onMouseEnter={e=>e.currentTarget.style.boxShadow="0 6px 24px rgba(15,23,42,.12)"}
              onMouseLeave={e=>e.currentTarget.style.boxShadow=""}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
              <div>
                <div style={{fontWeight:700,fontSize:13,color:"#0f172a",marginBottom:1}}>Daily Hours Trend</div>
                <div style={{fontSize:11,color:"#9ca3af"}}>Mar 8–14 · hours/day</div>
              </div>
              <span style={{fontSize:10,color:"#1a56db",fontWeight:600,background:"#eff6ff",padding:"3px 8px",borderRadius:8}}>↗ view log</span>
            </div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:10}}>
              <div>
                <div style={{fontSize:26,fontWeight:800,color:"#1a56db",letterSpacing:-.5}}>{sparkPts.reduce((s,v)=>s+v,0)}h</div>
                <div style={{fontSize:11,color:"#9ca3af"}}>this week</div>
              </div>
              <div style={{textAlign:"right",padding:"4px 8px",background:"#f0fdf4",borderRadius:8}}>
                <div style={{fontSize:13,fontWeight:800,color:"#059669"}}>↑ 12%</div>
                <div style={{fontSize:9,color:"#9ca3af"}}>vs last week</div>
              </div>
            </div>
            <SvgSparkline points={sparkPts} color="#1a56db" height={56}/>
            <div style={{display:"flex",justifyContent:"space-between",marginTop:6}}>
              {["M","T","W","T","F","S","S"].map((d,i)=>(
                <div key={i} style={{textAlign:"center",flex:1}}>
                  <div style={{fontSize:10,fontWeight:700,color:"#1a56db"}}>{sparkPts[i]}h</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1.2fr",gap:14,marginBottom:14}}>
          ...
          <Card style={{cursor:"pointer"}} onClick={()=>goPage("timelog")}
              onMouseEnter={e=>e.currentTarget.style.boxShadow="0 4px 20px rgba(15,23,42,.1)"}
              onMouseLeave={e=>e.currentTarget.style.boxShadow=""}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
              <div>
                <div style={{fontWeight:700,fontSize:13,color:"#0f172a",marginBottom:2}}>Hours by {isAdmin?"Team":"Member"}</div>
                <div style={{fontSize:11,color:"#9ca3af"}}>{isAdmin?"Logged hours per team":"Logged hours per person"}</div>
              </div>
              <span style={{fontSize:10,color:"#1a56db",fontWeight:600,background:"#eff6ff",padding:"3px 8px",borderRadius:8}}>↗ view log</span>
            </div>
            <ColumnChart
          ...
          <Card>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
              <div>
                <div style={{fontWeight:700,fontSize:13,color:"#0f172a",marginBottom:2}}>Activity Types</div>
                <div style={{fontSize:11,color:"#9ca3af"}}>Count by work type · click to drill</div>
              </div>
            </div>
            <LollipopChart
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1.4fr 1.4fr",gap:14,marginBottom:14}}>
          ...
          <Card>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
              <div>
                <div style={{fontWeight:700,fontSize:13,color:"#0f172a",marginBottom:2}}>Time by Category</div>
                <div style={{fontSize:11,color:"#9ca3af"}}>Hours distribution · click to drill</div>
              </div>
              <div style={{fontSize:13,fontWeight:800,color:"#1a56db"}}>{fmtM(totalMins)}</div>
            </div>
              ...
              <div style={{width:"100%",display:"flex",flexDirection:"column",gap:4}}>
                {logsByCat.map(d=>(
                  <div key={d.label} onClick={()=>setDashDrill({type:"logs",label:`${d.fullLabel} — Time Entries`,data:dashLogs.filter(l=>l.cat===d.fullLabel)})}
                    ...
                    <span style={{fontSize:11,color:"#374151",fontWeight:600,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{d.fullLabel}</span>
                    <span style={{fontSize:12,fontWeight:800,color:d.color,flexShrink:0}}>{d.val}h</span>
                    <span style={{fontSize:9,color:"#9ca3af",flexShrink:0,minWidth:28,textAlign:"right"}}>{Math.round(d.val/Math.max(totalMins/60,1)*100)}%</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {(()=>{
            ...
            return (
              <Card>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                  <div>
                    <div style={{fontWeight:700,fontSize:13,color:"#0f172a"}}>🏆 Top Activities</div>
                    <div style={{fontSize:11,color:"#9ca3af",marginTop:1}}>Most logged this month</div>
                  </div>
          ...
          {(()=>{
            ...
            return (
              <Card>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                  <div>
                    <div style={{fontWeight:700,fontSize:13,color:"#0f172a"}}>⏰ Most Time-Consuming</div>
                    <div style={{fontSize:11,color:"#9ca3af",marginTop:1}}>Highest hours logged</div>
                  </div>
  ...
  const PageReports = () => {
    ...
    return (
      <div>
        ...
        <div style={{fontSize:12,color:"#6b7280",marginBottom:20}}>Utilization, efficiency, cost analytics · March 2025 · {isAdmin?"All teams":"Your team"}</div>
        <Tabs tabs={TABS} active={reportTab} onChange={setReportTab}/>
        {/* ══ EFFICIENCY & TRENDS TAB ══ */}
        {reportTab==="efficiency" && (
          <div>
            {!isAdmin && <InfoBanner color="#1e40af" bg="#eff6ff" border="#bfdbfe" icon="🔒">Showing data for <strong>{user.team}</strong> team only.</InfoBanner>}

            {/* KPI Strip */}
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:16}}>
              {[
                {label:"Avg Time / Activity",   val:fmtM(curAvgPerAct),   sub:"current period",   c:"#1a56db",bg:"#eff6ff",icon:"⏱"},
                ...
                <Card key={s.label} style={{padding:14,borderLeft:`3px solid ${s.c}`}}>
                  <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:8}}>
                    ...
                    <div style={{fontSize:9,fontWeight:700,color:"#9ca3af",textTransform:"uppercase",letterSpacing:.8,lineHeight:1.3}}>{s.label}</div>
                  </div>
            ...
            <Card style={{marginBottom:14}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
                <div>
                  <div style={{fontWeight:700,fontSize:14,color:"#0f172a"}}>Average Time per Activity — 6-Month Trend</div>
                  <div style={{fontSize:11,color:"#6b7280",marginTop:2}}>
                    Target: reduce 10% annually (~0.87%/month) · Darker bars = current period · Dashed line = target trajectory
                  </div>
                </div>
                <div style={{background:"#f0fdf4",border:"1px solid #a7f3d0",borderRadius:8,padding:"6px 12px",fontSize:11,color:"#059669",fontWeight:700}}>
                  Target: -10%/yr
                </div>
              </div>
              {[
                {label:"All Activities",   key:"avgMinsPerAct",      color:"#1a56db"},
                ...
                return (
                  <div key={row.label} style={{marginBottom:18}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                      ...
                      <div style={{display:"flex",gap:10,alignItems:"center"}}>
                        <span style={{fontSize:11,color:"#6b7280"}}>Current: <strong style={{color:row.color}}>{fmtM(vals[vals.length-1])}</strong></span>
                        ...
                        <span style={{fontSize:10,color:"#9ca3af"}}>Target: {fmtM(monthlyTarget(baseVal,5))}</span>
                      </div>
                    </div>
                      ...
                      {allPeriods.map((p,i)=>{
                        ...
                        return (
                          <div key={p.period} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2,position:"relative"}}>
                            {/* Target dot */}
                            <div style={{position:"absolute",bottom:targetH+6,left:"50%",transform:"translateX(-50%)",width:6,height:6,borderRadius:"50%",background:"#94a3b8",border:"1px solid #fff",zIndex:2}}/>
                            {/* Bar */}
                            ...
                            <div style={{position:"absolute",bottom:0,fontSize:9,color:isLast?"#1e293b":"#94a3b8",fontWeight:isLast?700:400}}>{p.period.slice(0,3)}</div>
                          </div>
                        );
                      })}
                      {/* Baseline */}
                      <div style={{position:"absolute",bottom:6,left:0,right:0,height:1,background:"#e2e8f0"}}/>
                    </div>
                    <div style={{fontSize:9,color:"#9ca3af",marginTop:2,display:"flex",alignItems:"center",gap:4}}>
            ...
            <Card style={{marginBottom:14}}>
              <div style={{fontWeight:700,fontSize:14,color:"#0f172a",marginBottom:4}}>Engineer Efficiency — Avg Time per Activity Type</div>
              <div style={{fontSize:11,color:"#6b7280",marginBottom:14}}>How long each engineer spends on different activity types · click engineer to see time logs</div>
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5}}>
                  ...
                  <tbody>
                    {scopeUsers2.map((u,i)=>{
                      ...
                      return (
                        <tr key={u.id} style={{background:i%2===0?"#fff":"#fafbfc",borderBottom:"1px solid #f1f5f9",cursor:"pointer"}}
                          onClick={()=>setDashDrill({type:"logs",label:`${u.name} — All Entries`,data:logs.filter(l=>l.userId===u.id)})}>
                          <TD>
                            <div style={{display:"flex",alignItems:"center",gap:8}}>
                              <div style={{width:26,height:26,borderRadius:"50%",background:`${col}22`,color:col,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:800,flexShrink:0}}>{u.name.split(" ").map(n=>n[0]).join("")}</div>
                              <span style={{fontWeight:700}}>{u.name}</span>
                            </div>
                          </TD>
                          <TD><TPill t={u.team}/></TD>
                          <TD s={{fontWeight:800,color:"#1a56db",fontSize:13}}>{fmtM(uAvg)||"—"}</TD>
                          ...
                          <TD>
                            {uAvg>0&&curAvgPerAct>0?(
                              <span style={{fontSize:11,fontWeight:700,background:vsTeam>10?"#fef2f2":vsTeam>0?"#fffbeb":vsTeam<-10?"#f0fdf4":"#f7f9fc",color:vsTeam>10?"#dc2626":vsTeam>0?"#d97706":vsTeam<-10?"#059669":"#374151",padding:"2px 8px",borderRadius:8}}>
                                {vsTeam>0?"+":""}{vsTeam}%
                              </span>
                            ):"—"}
                          </TD>
            ...
            <Card>
              <div style={{fontWeight:700,fontSize:14,color:"#0f172a",marginBottom:4}}>Activity Deep-Dive — Time Variance by Engineer</div>
              <div style={{fontSize:11,color:"#6b7280",marginBottom:14}}>See how different engineers handle the same activity type — identifies training or process gaps</div>
              {scopeActs2.filter(a=>{
                ...
                const col=tCol(a.team);
                return (
                  <div key={a.id} style={{marginBottom:16,padding:"12px 14px",borderRadius:10,background:"#f8fafc",border:"1px solid #e8edf5"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                      <div>
                        <div style={{fontWeight:700,fontSize:13,color:"#0f172a"}}>{a.name}</div>
                        <div style={{fontSize:10,color:"#9ca3af",marginTop:2}}>{a.ticketNo} · {a.type} · Est: {fmtM(a.estMins)} · Total logged: {fmtM(totalLogged)}</div>
                      </div>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:6}}>
                      {engArr.map(e=>(
                        <div key={e.name} style={{display:"flex",alignItems:"center",gap:10}}>
                          <div style={{width:110,fontSize:11,fontWeight:600,color:"#374151",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",flexShrink:0}}>{e.name.split(" ")[0]}</div>
                          <div style={{flex:1,height:12,background:"#e8edf5",borderRadius:4,overflow:"hidden",position:"relative"}}>
                          ...
                          <div style={{width:40,textAlign:"right",fontSize:10,color:"#9ca3af",flexShrink:0}}>{e.entries} logs</div>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        {reportTab==="cost" && (
          <div>
            {!isAdmin && <InfoBanner color="#1e40af" bg="#eff6ff" border="#bfdbfe" icon="🔒">Cost data for <strong>{user.team}</strong> team. Admin sees org-wide.</InfoBanner>}
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:16}}>
              {[
                {label:"Total Period Cost",   val:`$${Math.round(totalCost).toLocaleString()}`, sub:`${scopeLogs2.length} log entries`,     c:"#1a56db",bg:"#eff6ff",icon:"💰"},
            ...
            <div style={{display:"grid",gridTemplateColumns:"1.4fr 1fr",gap:14,marginBottom:14}}>
              <Card>
                <div style={{fontWeight:700,fontSize:14,color:"#0f172a",marginBottom:4}}>Cost per Ticket — 6-Month Trend</div>
                <div style={{fontSize:11,color:"#6b7280",marginBottom:14}}>Target: reduce 10% annually. Darker bar = current period.</div>
                  ...
                  {allPeriods.map((p,i)=>{
                    ...
                    return (
                      <div key={p.period} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3,position:"relative",height:140}}>
                        ...
                        <div style={{position:"absolute",top:0,fontSize:9,fontWeight:isLast?700:400,color:isLast?"#1e293b":"#9ca3af"}}>${val}</div>
                        <div style={{position:"absolute",bottom:0,fontSize:9,color:isLast?"#1a56db":"#94a3b8",fontWeight:isLast?700:400}}>{p.period.slice(0,3)}</div>
                      </div>
                    );
                  })}
                </div>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"#9ca3af",borderTop:"1px solid #f1f5f9",paddingTop:8}}>
              ...
              <Card>
                <div style={{fontWeight:700,fontSize:14,color:"#0f172a",marginBottom:4}}>Cost Breakdown by Type</div>
                <div style={{fontSize:11,color:"#6b7280",marginBottom:14}}>Total spend per activity type this period</div>
                {["Incident","Change","Project","BAU","Training","Meeting"].map(t=>{
                  ...
                  const col={Incident:"#dc2626",Change:"#d97706",Project:"#1a56db",BAU:"#059669",Training:"#0891b2",Meeting:"#7c3aed"}[t]||"#6b7280";
                  return (
                    <div key={t} style={{marginBottom:10}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                        ...
                        <div style={{display:"flex",gap:10,fontSize:11}}>
                          <span style={{color:"#9ca3af"}}>{tActs} act{tActs!==1?"s":""}</span>
            ...
            <Card style={{marginBottom:14,padding:0,overflow:"hidden"}}>
              <div style={{padding:"14px 18px",borderBottom:"1px solid #f1f5f9"}}>
                <div style={{fontWeight:700,fontSize:14,color:"#0f172a"}}>Cost per Engineer</div>
                <div style={{fontSize:11,color:"#6b7280",marginTop:2}}>Total cost of logged hours per engineer this period · hourly rates from User Management</div>
              </div>
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5}}>
                  <thead><tr>
                    ...
                    {scopeUsers2.sort((a,b)=>{
                      ...
                      return (
                        <tr key={u.id} style={{background:i%2===0?"#fff":"#fafbfc",borderBottom:"1px solid #f1f5f9"}}
                          ...
                          <TD><TPill t={u.team}/></TD>
                          <TD s={{fontWeight:700,color:"#059669"}}>{u.costPerHour?`$${u.costPerHour}`:<span style={{color:"#9ca3af",fontStyle:"italic"}}>not set</span>}</TD>
                          <TD s={{fontWeight:600,color:"#1a56db"}}>{uHrs}h</TD>
                          <TD s={{fontWeight:800,fontSize:14,color:uCost>5000?"#dc2626":uCost>3000?"#d97706":"#374151"}}>{uCost?`$${Math.round(uCost).toLocaleString()}`:"—"}</TD>
                          ...
                          <TD s={{fontSize:11,color:"#6b7280"}}>
                            {topAct?<span>{topAct[1].name?.slice(0,25)}{topAct[1].name?.length>25?"…":""} <strong style={{color:"#1a56db"}}>${Math.round(topAct[1].cost)}</strong></span>:"—"}
                          </TD>
                        </tr>
                      );
                    })}
            ...
            <Card style={{padding:0,overflow:"hidden"}}>
              <div style={{padding:"14px 18px",borderBottom:"1px solid #f1f5f9"}}>
                <div style={{fontWeight:700,fontSize:14,color:"#0f172a"}}>Cost per Activity</div>
                <div style={{fontSize:11,color:"#6b7280",marginTop:2}}>Total and average cost per activity — sorted by highest cost first</div>
              </div>
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5}}>
                  <thead><tr>
                  ...
                  <tbody>
                    {scopeActs2.map(a=>{
                      ...
                      const col=tCol(a.team);
                      return (
                        <tr key={a.id} style={{background:i%2===0?"#fff":"#fafbfc",borderBottom:"1px solid #f1f5f9",cursor:"pointer"}}
                          onClick={()=>setDashDrill({type:"logs",label:`${a.name} — Entries`,data:aLogs})}>
                          <TD>
                            <div style={{fontWeight:700,fontSize:12,color:"#0f172a",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:180}}>{a.name}</div>
                            <div style={{fontSize:9,color:"#9ca3af"}}>{a.status}</div>
                          </TD>
                          <TD><code style={{background:"#f1f5f9",padding:"2px 6px",borderRadius:4,fontSize:11}}>{a.ticketNo||"—"}</code></TD>
                          <TD s={{fontSize:11}}>{a.type}</TD>
                          <TD><TPill t={a.team}/></TD>
                          <TD s={{color:"#6b7280",fontWeight:600}}>{estHrs}h</TD>
                          <TD s={{fontWeight:700,color:"#1a56db"}}>{aHrs}h</TD>
                          <TD s={{fontWeight:800,fontSize:14,color:aCost>2000?"#dc2626":aCost>1000?"#d97706":"#059669"}}>
                            {aCost?`$${Math.round(aCost).toLocaleString()}`:"—"}
                          </TD>
                          <TD s={{color:"#6b7280"}}>{aHrs?`$${Math.round(aCost/Math.max(aHrs,1))}/hr`:"—"}</TD>
                          <TD>
                            {aHrs>0?(
                              <span style={{fontSize:11,fontWeight:700,background:overEst?"#fef2f2":"#f0fdf4",color:overEst?"#dc2626":"#059669",padding:"2px 8px",borderRadius:8}}>
  ...
  const PageSettings = () => {
    ...
    return (
      <div>
        ...
        {settingsTab==="data" && (
          <Card>
            <div style={{fontWeight:700,fontSize:14,marginBottom:4}}>💾 Data Management</div>
            <div style={{fontSize:12,color:"#6b7280",marginBottom:16}}>Manage and persist application data</div>
            ...
            <div style={{borderTop:"1px solid #f1f5f9",paddingTop:16}}>
              <div style={{fontWeight:600,fontSize:13,marginBottom:12,color:"#0f172a"}}>Reset Data</div>
              <div style={{fontSize:12,color:"#6b7280",marginBottom:12}}>If you need to restore the original seed data and remove all custom users, teams, and settings:</div>
              <button 
                onClick={() => {
                ...
                style={{padding:"9px 16px",borderRadius:8,background:"#fef2f2",border:"1px solid #fca5a5",color:"#dc2626",fontWeight:600,fontSize:12,cursor:"pointer"}}>
                🔄 Reset to Seed Data
              </button>
            </div>

            <div style={{borderTop:"1px solid #f1f5f9",marginTop:16,paddingTop:16}}>
              <div style={{fontWeight:600,fontSize:13,marginBottom:12,color:"#0f172a"}}>Storage Info</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                <div style={{background:"#f7f9fc",border:"1px solid #e5e7eb",borderRadius:8,padding:12}}>
                  <div style={{fontSize:10,color:"#9ca3af",fontWeight:600,marginBottom:4}}>STORAGE TYPE</div>
                  <div style={{fontSize:13,fontWeight:700,color:"#0f172a"}}>Browser LocalStorage</div>
                  <div style={{fontSize:11,color:"#6b7280",marginTop:4}}>Data persists in your browser</div>
                </div>
                <div style={{background:"#f7f9fc",border:"1px solid #e5e7eb",borderRadius:8,padding:12}}>
                  <div style={{fontSize:10,color:"#9ca3af",fontWeight:600,marginBottom:4}}>AUTO-SAVE</div>
                  <div style={{fontSize:13,fontWeight:700,color:"#059669"}}>Real-time</div>
                  <div style={{fontSize:11,color:"#6b7280",marginTop:4}}>Changes saved instantly</div>
                </div>
              </div>
            </div>
          </Card>
          ...
          <ModalWrap title={uTarget?"✏️ Edit User":"👤 Add User"} onClose={()=>setUModal(false)}>
            <div style={{display:"grid",gap:13}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                ...
                {!uTarget && (
                  <Lbl t="Password *">
                    ...
                    <div style={{fontSize:10,color:"#9ca3af",marginTop:3}}>Password for this user to login (min 6 chars)</div>
                  </Lbl>
                )}
              ...
              <Lbl t="Hourly Cost ($/hr)">
                ...
                <div style={{fontSize:10,color:"#9ca3af",marginTop:3}}>Used for cost-per-ticket calculations in Reports. Leave 0 to exclude from cost analysis.</div>
              </Lbl>
              {isAdmin && uTarget && (
                <div style={{gridColumn:"1/-1"}}>
      ...
      <div>
        ...
        <div style={{marginBottom:20}}>
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
            <div>
              <div style={{fontSize:20,fontWeight:800,color:"#0f172a"}}>🧠 Intelligence Report</div>
              <div style={{fontSize:12,color:"#6b7280",marginTop:3}}>
            ...
            {/* Grade badge */}
            <div style={{textAlign:"center",background:`linear-gradient(135deg,${gradeColor},${gradeColor}cc)`,borderRadius:14,padding:"10px 24px",boxShadow:`0 4px 16px ${gradeColor}44`}}>
        ...
        {/* Urgent alert banner */}
        {critCount > 0 && (
          <div style={{background:"#fef2f2",border:"1px solid #fca5a5",borderLeft:"4px solid #dc2626",borderRadius:10,padding:"12px 16px",marginBottom:16,display:"flex",alignItems:"center",gap:12}}>
            ...
            <button onClick={()=>setActiveTab("deep")} style={{marginLeft:"auto",padding:"6px 16px",borderRadius:8,background:"#dc2626",color:"#fff",border:"none",fontWeight:700,fontSize:12,cursor:"pointer",flexShrink:0}}>
              View →
            </button>
          </div>
        )}
        ...
        {activeTab==="executive" && (
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1.2fr 1fr",gap:14,marginBottom:14}}>
              <Card>
                <div style={{fontWeight:700,fontSize:14,color:"#0f172a",marginBottom:4}}>Time Allocation vs Benchmark</div>
                <div style={{fontSize:11,color:"#6b7280",marginBottom:16}}>How your team spends time vs industry targets</div>
                {[
                  ...
                  {label:"Projects",       actual:projectPct,  target:35,  color:"#1a56db", note:"Invest"},
                  {label:"Automation",     actual:autoPct,     target:30,  color:"#7c3aed", note:"Increase"},
                  {label:"Training",       actual:trainingPct, target:10,  color:"#0891b2", note:"Increase"},
                  ...
                  return (
                    <div key={row.label} style={{marginBottom:10}}>
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:4}}>
                        ...
                        <div style={{display:"flex",gap:8,alignItems:"center"}}>
                          <span style={{color:"#9ca3af",fontSize:10}}>target {row.target}%</span>
              ...
              <Card>
                <div style={{fontWeight:700,fontSize:14,color:"#0f172a",marginBottom:4}}>Management Summary</div>
                <div style={{fontSize:11,color:"#6b7280",marginBottom:14}}>Key insights for leadership</div>
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  {[
                    {icon:"💡",color:"#1a56db",bg:"#eff6ff",
                    ...
                    <div key={i.title} style={{background:i.bg,borderRadius:10,padding:"10px 12px",border:`1px solid ${i.color}20`}}>
                      <div style={{display:"flex",gap:8,alignItems:"flex-start"}}>
                        ...
                        <div>
                          ...
                          <div style={{fontSize:11,color:"#374151",lineHeight:1.5}}>{i.body}</div>
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        {activeTab==="benchmarks" && (
          <div>
              ...
              {[
                ...
                <div key={s.label} style={{background:s.bg,border:`1px solid ${s.c}30`,borderRadius:12,padding:"16px 18px",display:"flex",alignItems:"center",gap:12}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:11,color:"#6b7280"}}>{s.label} metrics</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <Card style={{padding:0,overflow:"hidden"}}>
              <div style={{padding:"14px 18px",borderBottom:"1px solid #f1f5f9"}}>
                <div style={{fontWeight:700,fontSize:14,color:"#0f172a"}}>Full Benchmark Scorecard</div>
                <div style={{fontSize:11,color:"#6b7280",marginTop:2}}>Your actual vs industry best practice targets · Sources: ITIL v4, DORA, Google SRE, FinOps Foundation, Gartner</div>
              </div>
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                  <thead>
                  ...
                  <tbody>
                    {metrics.map((m,i)=>{
                      ...
                      return (
                        <tr key={m.key} style={{background:i%2===0?"#fff":"#fafbfc",borderBottom:"1px solid #f1f5f9"}}>
                          <TD>
                            <div style={{fontWeight:700,fontSize:12,color:"#0f172a"}}>{m.label}</div>
                            <div style={{fontSize:10,color:"#9ca3af",marginTop:1}}>{m.bench.desc}</div>
                          </TD>
                          <TD><span style={{fontSize:18,fontWeight:800,color:col}}>{m.actual}</span></TD>
                          <TD s={{fontWeight:600,color:"#374151"}}>{m.target}</TD>
                          <TD>
                            <span style={{fontSize:11,background:bg,color:col,padding:"3px 10px",borderRadius:10,fontWeight:700,whiteSpace:"nowrap"}}>
                              {scoreLabel(rating)}
                            </span>
                          </TD>
                          <TD s={{fontSize:10,color:"#6b7280"}}>{m.bench.source}</TD>
                          <TD s={{fontSize:11,color:rating==="elite"?"#059669":col,fontWeight:600}}>{gap}</TD>
                          <TD>
                            <button onClick={()=>setActiveTab("deep")}
                              style={{padding:"4px 10px",borderRadius:7,background:"#f1f5f9",border:"1px solid #e2e8f0",fontSize:11,fontWeight:600,cursor:"pointer",color:"#374151",whiteSpace:"nowrap"}}>
              ...
              {["Critical","High","Medium","Good"].map(p=>{
                ...
                return (
                  <Card key={p} style={{padding:12,borderLeft:`3px solid ${priColor(p)}`}}>
                    ...
                    <div style={{fontSize:10,color:"#9ca3af"}}>finding{cnt!==1?"s":""}</div>
                  </Card>
                );
              })}
            </div>
            ...
            {sortedRecs.map(rec=>(
              <div key={rec.id} style={{marginBottom:12}}>
                <div onClick={()=>setExpandedRec(expandedRec===rec.id?null:rec.id)}
                  style={{background:"#fff",border:`1px solid ${priColor(rec.priority)}25`,
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:5}}>
                      <span style={{fontWeight:800,fontSize:14,color:"#0f172a"}}>{rec.title}</span>
                      <span style={{fontSize:10,background:priBg(rec.priority),color:priColor(rec.priority),padding:"2px 9px",borderRadius:10,fontWeight:700,flexShrink:0}}>{rec.priority}</span>
                      <span style={{fontSize:10,background:"#f1f5f9",color:"#6b7280",padding:"2px 8px",borderRadius:10,flexShrink:0}}>{rec.framework}</span>
                    </div>
                    <div style={{fontSize:12,color:"#6b7280",lineHeight:1.6}}>{rec.finding}</div>

                    {expandedRec===rec.id && (
                      <div style={{marginTop:14}}>
                        {/* Root cause */}
                        <div style={{background:"#fff7ed",border:"1px solid #fed7aa",borderRadius:9,padding:"10px 14px",marginBottom:12}}>
                          <div style={{fontSize:11,fontWeight:700,color:"#c2410c",marginBottom:4}}>🔍 Root Cause Analysis</div>
                          <div style={{fontSize:12,color:"#374151",lineHeight:1.6}}>{rec.rootCause}</div>
                        </div>
                        {/* Recommendations */}
                        <div style={{background:priBg(rec.priority),border:`1px solid ${priColor(rec.priority)}25`,borderRadius:9,padding:"12px 14px",marginBottom:12}}>
                        ...
                        <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:10,alignItems:"start"}}>
                          <div style={{background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:8,padding:"8px 12px"}}>
                            <div style={{fontSize:10,fontWeight:700,color:"#64748b",marginBottom:2}}>📊 KPI Target</div>
                            <div style={{fontSize:12,color:"#1e293b",fontWeight:600}}>{rec.kpi}</div>
                          </div>
                          <div style={{background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:8,padding:"8px 12px"}}>
                            <div style={{fontSize:10,fontWeight:700,color:"#64748b",marginBottom:4}}>📚 Frameworks</div>
                            {rec.frameworks.map(f=>(
                              <div key={f} style={{fontSize:10,color:"#6b7280",marginBottom:2}}>• {f}</div>
                            ))}
                          </div>
                        </div>
                      </div>
                  ...
                  <span style={{fontSize:14,color:"#9ca3af",flexShrink:0,marginTop:4}}>{expandedRec===rec.id?"▲":"▼"}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {activeTab==="trends" && (
          <div>
            <InfoBanner color="#1e40af" bg="#eff6ff" border="#bfdbfe" icon="📈">
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
              {[
                ...
                {title:"Utilisation Trend",   key:"util",     target:80, color:"#1a56db", label:"% util",     good:"stable"},
                ...
                <Card key={chart.title}>
                  <div style={{fontWeight:700,fontSize:13,color:"#0f172a",marginBottom:2}}>{chart.title}</div>
                  <div style={{fontSize:11,color:"#9ca3af",marginBottom:14}}>Target: {chart.target}% · Trend: {chart.good==="down"?"Lower is better":chart.good==="up"?"Higher is better":"Stay in range"}</div>
                      ...
                      return (
                        <div key={d.period} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
                          ...
                          <div style={{width:"100%",maxWidth:32,position:"relative",height:80,display:"flex",alignItems:"flex-end"}}>
                            ...
                            {/* Target line */}
                            <div style={{position:"absolute",bottom:`${targetH}%`,left:0,right:0,height:1.5,background:"#374151",opacity:.4}}/>
                          </div>
                          <div style={{fontSize:9,color:isLast?"#1a56db":"#9ca3af",fontWeight:isLast?700:400}}>{d.period}</div>
                        </div>
                      );
                    })}
                  </div>
            ...
            <Card>
              <div style={{fontWeight:700,fontSize:14,color:"#0f172a",marginBottom:4}}>6-Month Metric Trajectory</div>
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                  ...
                  <tbody>
                    {[
                      ...
                      return (
                        <tr key={row.key} style={{background:ri%2===0?"#fff":"#fafbfc"}}>
                          ...
                          <TD><span style={{background:onTarget?"#f0fdf4":"#fef2f2",color:onTarget?"#059669":"#dc2626",padding:"2px 8px",borderRadius:8,fontWeight:700,fontSize:11}}>{onTarget?"✓ Met":"✗ Below"}</span></TD>
                        </tr>
                      );
                    })}
                  </tbody>
          ...
          <div>
            ...
            {[
              ...
              <div key={phase.period} style={{marginBottom:16}}>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                  <div style={{width:36,height:36,borderRadius:10,background:phase.bg,border:`1px solid ${phase.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>{phase.icon}</div>
                  <div>
                    <div style={{fontWeight:800,fontSize:14,color:"#0f172a"}}>{phase.period}</div>
                    <div style={{fontSize:11,color:"#6b7280"}}>{phase.focus}</div>
                  </div>
                </div>
                    ...
                    <tbody>
                      {phase.actions.map((a,i)=>(
                        <tr key={i} style={{borderBottom:"1px solid #f1f5f9",background:i%2===0?"#fff":"#fafbfc"}}>
                          <TD s={{width:30,textAlign:"center",color:"#9ca3af",fontWeight:700}}>{i+1}</TD>
                          <TD s={{fontWeight:500,color:"#1e293b"}}>{a.action}</TD>
                          <TD s={{fontSize:11,color:"#64748b",whiteSpace:"nowrap"}}>{a.owner}</TD>
                          <TD><span style={{fontSize:11,background:a.impact==="High"?"#fef2f2":"#fffbeb",color:a.impact==="High"?"#dc2626":"#d97706",padding:"2px 8px",borderRadius:8,fontWeight:700}}>{a.impact}</span></TD>
                          <TD><span style={{fontSize:11,background:a.effort==="Low"?"#f0fdf4":a.effort==="Medium"?"#fffbeb":"#fef2f2",color:a.effort==="Low"?"#059669":a.effort==="Medium"?"#d97706":"#dc2626",padding:"2px 8px",borderRadius:8,fontWeight:700}}>{a.effort}</span></TD>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
        ...
        <div style={{flexShrink:0,background:"rgba(255,255,255,.97)",borderBottom:"1px solid #dde3ed",padding:"11px 22px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            ...
            <div style={{fontSize:11,color:"#6b7280"}}>ISMS {user.team ? `› ${user.team}` : ""} › <span style={{color:"#0f172a",fontWeight:600}}>{PAGE_TITLE[page]}</span></div>
          </div>
          <div style={{display:"flex",gap:9}}>
...
export default function App() {
  ...
  return <PortalApp key={authUser.id} initialUser={authUser} allUsersInit={allUsersForLogin}/>;
}
