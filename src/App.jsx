...
function PortalApp({initialUser, allUsersInit}) {
  ...
  const buildPrev = (rows, fieldMap) => rows.slice(0,8).map((r,i)=>{
    const raw=fieldMap.estMins?r[fieldMap.estMins]:""; const hrs=parseFloat(raw)||8;
    const estMins=hrs<50?Math.round(hrs*60):Math.round(hrs);
    const type=r[fieldMap.type]||"BAU";
    const nt=/proj/i.test(type)?"Project":/inc/i.test(type)?"Incident":/chg|change/i.test(type)?"Change":/train/i.test(type)?"Training":"BAU";
    const prio=r[fieldMap.priority]||"Medium";
    const np=/crit/i.test(prio)?"Critical":/high/i.test(prio)?"High":/low/i.test(prio)?"Low":"Medium";
    const wn=r[fieldMap.workNature]||"";
    return {
      _row:i+2, name:r[fieldMap.name]||"", type:nt,
      cat:r[fieldMap.cat]||"Maintenance", priority:np, estMins,
      ticketNo:r[fieldMap.ticketNo]||"", jira:r[fieldMap.jira]||"",
      nature:/react/i.test(r[fieldMap.nature]||"")?"Reactive":"Proactive",
      workNature:/inc/i.test(wn)?"Incident":/chg|change/i.test(wn)?"Change":"Request",
  ...
  const runActImport = () => {
    ...
    setTimeout(()=>{
      ...
      const newActs = importRows.map(r=>{
        ...
        const np=/crit/i.test(prio)?"Critical":/high/i.test(prio)?"High":/low/i.test(prio)?"Low":"Medium";
        const wn=r[importMap.workNature]||"";
        return {
          ...
          priority:   np,
          estMins,
          ticketNo:   r[importMap.ticketNo]||"",
          jira:       r[importMap.jira]||r[importMap.ticketNo]||"",
          nature:     /react/i.test(r[importMap.nature]||"")?"Reactive":"Proactive",
          workNature: /inc/i.test(wn)?"Incident":/chg|change/i.test(wn)?"Change":"Request",
          desc:       r[importMap.desc]||"",
          status:     "Active",
          date:       today(),
          team:       user.team,
        };
  ...
  const dlTemplate = () => {
    const rows = [
      "Activity Name,Type,Category,Priority,Est. Hours,Ticket / Ref,JIRA Ref,Nature,Work Nature,Description",
      "Firewall Rule Review,Change,Security & Compliance,High,4,CHG-0001,,Proactive,Change,Review and update firewall rules",
      "Daily Health Checks,BAU,Monitoring & Alerting,Low,1,,,Proactive,Request,Daily server health monitoring",
      "P1 Incident Response,Incident,Maintenance,Critical,8,INC-0001,,Reactive,Incident,Respond to P1 production outage",
      "Cloud Cost Optimisation,Project,Infrastructure Build,Medium,16,REQ-0001,,Proactive,Request,Review and optimise cloud spend",
    ];
    const csv = rows.join("\n");
    const blob = new Blob([csv], {type:"text/csv"});
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = "isms_activities_template.csv"; a.click();
  ...
  const resetImport = () => {
    ...
    setImportPrev([]); setImportResult(null);
  };
  ...
  const saveLog = () => {
    ...
    if(target){
      ...
      setLogs(p=>[{id:uid("TL"),date:lForm.date,userId:user.id,member:user.name,team:act.team,actId:lForm.actId,activity:act.name,type:act.type,cat,mins:+lForm.mins,notes:lForm.notes,ticketRef,nature,workType,workNature},...p]);
      ...
      <Card style={{padding:0,overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5,fontFamily:"var(--font-sans)"}}>
            <thead>
              <tr>
                <TH c="ID"/><TH c="Activity"/><TH c="Team"/><TH c="Nature"/>
                <TH c="Work Type"/><TH c="Ticket"/><TH c="Type"/><TH c="Category"/>
                <TH c="Est. Time"/><TH c="Logged"/><TH c="Progress"/><TH c="Priority"/>
                <TH c="Status"/>{isMgr && <TH c="Actions"/>}
              </tr>
            </thead>
            <tbody>
              {filtActs.map(a=>{
                ...
                return (
                  <tr key={a.id} className="trow-hover">
                    ...
                    <TD><div style={{fontWeight:600,color:"var(--text-1)"}}>{a.name}</div><div style={{fontSize:10,color:"var(--text-4)",marginTop:2,fontFamily:"var(--font-mono)"}}>{a.jira}</div></TD>
              ...
              {filtActs.length===0 && (
                <tr><td colSpan={isMgr?14:13} style={{textAlign:"center",padding:50,color:"var(--text-4)",fontSize:13}}>
              ...
              {importStep==="upload" && (
                <div>
                  <div
                    ...
                    <div style={{fontSize:12,color:"var(--text-3)",marginBottom:14}}>In Excel: <strong>File → Save As → CSV (Comma delimited)</strong> · Tab-separated also works</div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                    ...
                    <div style={{background:"var(--surface-2)",border:"1px solid var(--border)",borderRadius:"var(--radius-md)",padding:"16px 18px"}}>
                      ...
                      {[
                        ["Auto-detect","Column headers matched by keyword — names don't need to be exact."],
                        ["Est. Hours","Enter effort in hours. 8 = 1 day, 40 = 1 week. Decimals OK."],
                        ["Type values","Project · Change · BAU · Incident · Training · Meeting"],
                        ["Priority","Critical · High · Medium · Low (auto-detected from text)"],
                        ["Your team",`All imported activities are added to: ${user.team||"your team"}`],
                      ].map(([t,d])=>(
              ...
              {(importStep==="mapping"||importStep==="importing") && (
                <div>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    ...
                    <button onClick={runActImport} disabled={!importMap.name||importStep==="importing"}
                ...
                <div style={{marginTop:10,background:"var(--surface-2)",border:"1px solid var(--border)",borderRadius:"var(--radius-md)",padding:"10px 14px",display:"flex",alignItems:"flex-start",gap:12}}>
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{display:"flex",gap:12,marginTop:6}}>
                      <span style={{fontSize:11,color:"var(--text-4)"}}>Est: <strong style={{color:"var(--text-2)"}}>{fmtM(selAct.estMins)}</strong></span>
