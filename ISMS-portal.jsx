...
const SEED_USERS = [
  { id:"u0",  name:"Sarah Mitchell", team:null,       role:"admin",   title:"ISMS Administrator"  },
  { id:"u1",  name:"Alex Chen",    team:"Network",  role:"manager", title:"Network Lead"        },
  { id:"u2",  name:"Maria Santos", team:"Security", role:"manager", title:"Security Lead"       },
  { id:"u3",  name:"James Park",   team:"Compute",  role:"manager", title:"Compute Lead"        },
  { id:"u4",  name:"Priya Nair",   team:"Cloud",    role:"manager", title:"Cloud Lead"          },
  { id:"u5",  name:"David Kim",    team:"Storage",  role:"manager", title:"Storage Lead"        },
  { id:"u6",  name:"Lisa Johnson", team:"Database", role:"manager", title:"Database Lead"       },
  { id:"u7",  name:"Tom Wilson",   team:"Network",  role:"member",  title:"Network Engineer"    },
  { id:"u8",  name:"Emma Davis",   team:"Security", role:"member",  title:"Security Analyst"    },
  { id:"u9",  name:"Mike Brown",   team:"Compute",  role:"member",  title:"Virtualisation Eng." },
  { id:"u10", name:"Aisha Malik",  team:"Cloud",    role:"member",  title:"Cloud Engineer"      },
  { id:"u11", name:"Ravi Sharma",  team:"Storage",  role:"member",  title:"Storage Engineer"    },
  { id:"u12", name:"Jin Park",     team:"Database", role:"member",  title:"DBA Analyst"         },
];
...
export default function App() {
  ...
  const [user,       setUser]       = useState(SEED_USERS[0]); // start logged in as Alex Chen
  ...
  const isMgr   = user.role === "manager";  // manager = team-scoped
  const isMgrOrAdmin = isMgr || isAdmin;     // has manager-level nav
  ...
  const loggable  = useMemo(()=>acts.filter(a=>a.team===user.team&&a.status!=="Done"),[acts,user,isAdmin]);
  const teamLogs  = useMemo(()=>isAdmin ? logs : logs.filter(l=>l.team===user.team),[logs,user,isAdmin]);
  const myLogs    = useMemo(()=>logs.filter(l=>l.userId===user.id),[logs,user]);
    ...
    <div style={{display:"flex",height:"100vh",fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#f0f4f8",overflow:"hidden",position:"relative"}}>
      ...
      <div style={{width:236,minWidth:236,height:"100vh",background:"#ffffff",borderRight:"1px solid #eaeef4",display:"flex",flexDirection:"column",overflow:"visible",flexShrink:0,boxShadow:"2px 0 16px rgba(15,23,42,.05)",zIndex:100,fontFamily:"'DM Sans',sans-serif"}}>
        ...
        <div style={{padding:"10px 12px",borderBottom:"1px solid #eaeef4",flexShrink:0,position:"relative"}}>
          ...
          {showSwitch && (
            <div style={{position:"absolute",top:"100%",left:8,right:8,background:"#fff",borderRadius:10,border:"1px solid #dde3ed",boxShadow:"0 8px 24px rgba(15,23,42,.15)",zIndex:400,maxHeight:300,overflowY:"auto"}}>
              <div style={{fontSize:9,fontWeight:700,color:"#9ca3af",letterSpacing:1.5,textTransform:"uppercase",padding:"8px 10px 4px"}}>Administrator</div>
              {allUsers.filter(u=>u.role==="admin").map(u=>(
                <div key={u.id} onClick={()=>{setUser(u);setPage("dashboard");setShowSwitch(false);}} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",cursor:"pointer",background:user.id===u.id?"#ede9fe":"transparent"}}>
                  <div style={{width:22,height:22,borderRadius:"50%",background:"#6366f1",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:800,flexShrink:0}}>{u.name.split(" ").map(n=>n[0]).join("")}</div>
                  <div style={{fontSize:11,fontWeight:600,color:"#0f172a"}}>{u.name} <span style={{color:"#6366f1",fontSize:10}}>(Admin)</span></div>
                </div>
              ))}
              <div style={{fontSize:9,fontWeight:700,color:"#9ca3af",letterSpacing:1.5,textTransform:"uppercase",padding:"8px 10px 4px",borderTop:"1px solid #e5e7eb",marginTop:2}}>Managers</div>
              {allUsers.filter(u=>u.role==="manager").map(u=>{
                const c=tCol(u.team);
                return (
                  <div key={u.id} onClick={()=>{setUser(u);setPage("dashboard");setShowSwitch(false);}} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",cursor:"pointer",background:user.id===u.id?`${c}15`:"transparent"}}>
                    <div style={{width:22,height:22,borderRadius:"50%",background:c,color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:800,flexShrink:0}}>{u.name.split(" ").map(n=>n[0]).join("")}</div>
                    <div style={{fontSize:11,fontWeight:600,color:"#0f172a"}}>{u.name} <span style={{color:c,fontSize:10}}>({u.team})</span></div>
                  </div>
                );
              })}
              <div style={{fontSize:9,fontWeight:700,color:"#9ca3af",letterSpacing:1.5,textTransform:"uppercase",padding:"8px 10px 4px",borderTop:"1px solid #e5e7eb",marginTop:2}}>Members</div>
              {allUsers.filter(u=>u.role==="member").map(u=>{
                const c=tCol(u.team);
                return (
                  <div key={u.id} onClick={()=>{setUser(u);setPage("dashboard");setShowSwitch(false);}} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",cursor:"pointer",background:user.id===u.id?`${c}15`:"transparent"}}>
                    <div style={{width:22,height:22,borderRadius:"50%",background:`${c}22`,color:c,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:800,flexShrink:0}}>{u.name.split(" ").map(n=>n[0]).join("")}</div>
                    <div style={{fontSize:11,fontWeight:600,color:"#0f172a"}}>{u.name} <span style={{color:"#9ca3af",fontSize:10}}>({u.team})</span></div>
                  </div>
                );
              })}
            </div>
